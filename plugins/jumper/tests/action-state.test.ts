...
test("Jumper composes observed session readiness and history with action availability", async () => {
  ...
  await plugin.activate({ withEvent: (_event: unknown, registration: unknown) => registration, locale: "en", contributions: { commands: { register }, headerActions: { register }, agentTools: { register: () => ({ dispose() {} }) } },
