...
export function subtitle(ctx: AnnotationContext, item: PluginAnnotation, books: Books): string {
  return `${books.get(item.bookId)?.title ?? tr(ctx.locale, "missingBook")} · ${tr(ctx.locale, item.kind)}`;
}
export async function readBooks(ctx: AnnotationContext, items: PluginAnnotation[]): Promise<Books> {
