...
import type { PluginContext, PluginView, PluginReactionEvent, SettingsObservation } from "@read-aware/plugin-types";
...
test("current workspace uses host observation, preserves last values on failure and releases its subscription", async () => {
  let handler!: (value: SettingsObservation, delivery?: PluginReactionEvent) => Promise<void>, disposed = 0;
  ...
  const delivery = { reaction: { id: "workspace-observation", status: "ready" as const } };
  const ctx = { withEvent: (event: PluginReactionEvent) => { expect(event).toBe(delivery); return ctx; }, locale: "en", domains: { settings: { queries: {
  ...
  await handler({ source: "remote", origin: null, status: "ready", snapshot: { ...state, revision: 4, settings: [{ ...state.settings[0], value: "list" }] } }, delivery);
  expect(JSON.stringify(published[published.length - 1])).toContain("list");
  await handler({ source: "local", origin: null, status: "error", revision: 5, code: "db/locked" }, delivery);
  ...
  await handler({ source: "restore", origin: null, status: "ready", snapshot: state }, delivery);
