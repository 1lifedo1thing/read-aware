...
async function prerequisites(ctx, bookId, mode, retryId, maxChapters = 20) {
  ...
  const states2 = ["satisfied", "unconfigured", "unavailable", "unknown"];
  return { kind: "detail", title: t[0], content: [
    { kind: "text", text: t[1] },
    { kind: "keyValue", rows: result.conditions.map((item) => ({ label: t[2 + kinds.indexOf(item.kind)] ?? t[0], value: t[10 + states2.indexOf(item.state)] })) },
    ...result.conditions.filter((item) => item.reason === "graph-boundary-unavailable").map(() => ({ kind: "text", text: taskBudgetWords(ctx.locale).boundary })),
    ...result.conditions.filter((item) => item.errorCode).map((item) => ({ kind: "error", code: item.errorCode }))
  ], actions: [
...
async function graphTasksView(ctx, bookId) {
  ...
  return liveMemoryView(ctx, { kind: "graphTasks", bookId }, t[0], (result) => {
    ...
    if (ctx.domains.memory.commands)
      actions.push({ id: "start", label: t[1], icon: "play", run: async () => ({ view: await prerequisites(ctx, bookId, "catch-up") }) }, { id: "rebuild", label: t[2], icon: "arrows-clockwise", run: async () => ({ view: await prerequisites(ctx, bookId, "rebuild") }) });
...
async function graphTaskView(ctx, bookId, taskId) {
  ...
  return liveMemoryView(ctx, { kind: "graphTask", bookId, taskId }, t[0], (result) => {
    ...
    if (ctx.domains.memory.commands && ["failed", "cancelled", "partial", "unavailable"].includes(task.status))
      actions.push({ id: "retry", label: t[3], icon: "arrows-clockwise", run: async () => ({ view: await prerequisites(ctx, bookId, task.mode, taskId, task.maxChapters) }) });
...
async function conversationControls(ctx, input, title) {
  ...
  return { ...render(initial), live: { subscribe(channel) {
    ...
    const subscription = domain.events.observeRuntime(async (state, delivery) => {
      if (!disposed && delivery?.reaction?.status !== "cycle")
        await ctx.withEvent(delivery).services.ui.publishView(channel, { revision: ++revision, view: render(state) });
