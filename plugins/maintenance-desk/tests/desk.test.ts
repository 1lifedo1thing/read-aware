...
function fixture() {
  ...
  const ctx = {
    ...
    services: { session: { operationAvailability: async (query: { operation: string }) => ({ state: "unknown", conditions: [{ state: "unknown", reason: query.operation === "diagnostics.verifyProjections" ? "projection-log-completeness-not-checked" : "host-flow-user-confirmation-required" }] }) }, maintenance: { requestConnectionTest, requestBackup }, diagnostics: { requestReport, verifyProjections, requestProjectionRepair },
...
describe("public API composition", () => {
  ...
  test("projection check stays in a live view, reports drift counts, and never repairs", async () => {
    const f = fixture(); plugin.activate(f.ctx);
    const connection = await select(view(await f.command().run()), "connection");
    expect(JSON.stringify(view(await action(connection, "prerequisites").run()))).toContain("Your confirmation is still required");
    expect(f.requestConnectionTest).not.toHaveBeenCalled();
