...
test("classification changes require an exact scoped revision and copy known fields", () => {
  ...
  expect(normalizeBookClassification({ ...input, narrativity: "narrative", spoilerSensitive: false }).spoilerSensitive).toBe(false);
  for (const raw of [null, [], {}, { ...input, bookId: " " }, { ...input, bookId: "a".repeat(257) },
    { ...input, narrativity: null }, { ...input, spoilerSensitive: "false" }, { ...input, narrativity: "poetry" }, { ...input, expectedRevision: `mem1:${"a".repeat(64)}` },
