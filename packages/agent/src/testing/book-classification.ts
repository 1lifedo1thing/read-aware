...
export function createBookClassificationFixture(books: BookOverview[]): RuntimeDeps["bookClassification"] {
  const versions = new Map<string, { flavor: BookOverview["narrativity"]; spoilerSensitive?: boolean; revision: string }>();
  ...
  const inspect = async (bookId: string, signal?: AbortSignal): Promise<BookClassificationSnapshot | null> => {
    ...
    if (!version || version.flavor !== book.narrativity || version.spoilerSensitive !== book.spoilerSensitive) {
      version = { flavor: book.narrativity, spoilerSensitive: book.spoilerSensitive, revision: `bcl1:${Array.from(crypto.getRandomValues(new Uint8Array(32)), byte => byte.toString(16).padStart(2, "0")).join("")}` };
      versions.set(bookId, version);
    }
    return { bookId, narrativity: book.narrativity ?? null, spoilerSensitive: book.spoilerSensitive ?? null, revision: version.revision };
  };
  return { inspect, change: async (input, signal) => {
    ...
    if (change.spoilerSensitive !== undefined) books.find(book => book.id === change.bookId)!.spoilerSensitive = change.spoilerSensitive;
