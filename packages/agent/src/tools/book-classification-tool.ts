...
export function buildBookClassificationTool(scope: ThreadScope, deps: RuntimeDeps): AgentTool {
  return { name: "classify_book", label: "Book classification", executionMode: "sequential",
    description: "Inspect or explicitly change a book's narrative/expository classification. Inspect first and pass that exact revision; every change requires user approval. Narrativity controls people/events versus concept digests. The independent spoilerSensitive flag protects fictional plot revelations; factual history, politics, biography and memoir default to false even when narrative. Classification affects future graph/prompt assembly, does not instantly rebuild digests, cancel in-flight work or erase past answers. Never reclassify just to bypass a spoiler fence. Book threads may only manage the current book; global threads require a discovered book ID. Conflicts require fresh inspection and renewed approval, never blind retry. Manual correction is allowed when automatic memory building is disabled.",
    parameters: Type.Object({ action: Type.Union([Type.Literal("inspect"), Type.Literal("classify")]), bookId: Type.Optional(Type.String({ minLength: 1, maxLength: 256 })),
      spoilerSensitive: Type.Optional(Type.Boolean()),
    ...
    execute: async (toolCallId, params, signal, onUpdate) => {
      ...
      const raw = params as { action: string; bookId?: string; narrativity?: DigestFlavor; spoilerSensitive?: boolean; expectedRevision?: string };
      ...
      if (!["inspect", "classify"].includes(raw.action) || Object.keys(raw).some(key => !["action", "bookId", ...(raw.action === "classify" ? ["narrativity", "spoilerSensitive", "expectedRevision"] : [])].includes(key))) throw new AppError("memory/invalid-input", "Invalid classification action");
      const change = raw.action === "inspect" ? null : normalizeBookClassification({ bookId, narrativity: raw.narrativity!, ...(raw.spoilerSensitive !== undefined ? { spoilerSensitive: raw.spoilerSensitive } : {}), expectedRevision: raw.expectedRevision! });
      ...
      const { answer, details } = await requestUserInteraction({ deps, toolCallId, threadKey: threadScopeKey(scope), signal, onUpdate,
        request: { kind: "permission", action: "classify-book", subject: `${book.title}\n${bookId}\n${snapshot.narrativity ?? "unclassified"} -> ${change.narrativity}${change.spoilerSensitive !== undefined ? `\nSpoiler protection: ${change.spoilerSensitive}` : ""}` } });
