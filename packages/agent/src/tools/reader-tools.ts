...
export function buildReaderTools(scope: ThreadScope, deps: RuntimeDeps, state?: AgentTurnState): AgentTool[] {
  ...
  const session: AgentTool = {
    ...
    description: "Read the actual active reader status, versioned location, visible text, selection, history availability and current pagination. change reports the origin and reason of the last host publication; command acknowledgements are stamped by the host, while unattributed renderer or lifecycle feedback uses system. It describes a coalescible snapshot update, not human presence, a complete causal audit or replayable events; sessionId is the generation identity. readerDemand reports recent render/relocation activity and its 1.5-second idleAt deadline; it is the same signal used to yield background text extraction, not proof of human presence or a durable event log. visibleTextState distinguishes available, rendered empty, and unavailable text; PDF text includes intersecting text runs, not OCR or exact glyph/overlay visibility. truncated means the preview is incomplete. pagination.section is zero-based source order including non-linear sections. pagination.screen counts viewport/spread steps only within the current reflowable section, not columns, printed page labels or whole-book pages; null for fixed layout, continuous scroll or transient geometry. It changes with window/font/layout and is not a persistent navigation target. selection.range can be passed unchanged to read_book_range. A book-scoped turn does not expose another book's viewport or selection.",
    parameters: Type.Object({}),
