...
function check(name: string, scope: ThreadScope, deps: RuntimeDeps, context: ReturnType<typeof readContext>, turnState?: AgentTurnState): ToolAvailability {
  if (name === "web_search" || name === "web_fetch") return deps.web?.configured(name === "web_fetch" ? "fetch" : "search") ? available : unavailable("search-not-configured-or-provider-unsupported; check the selected provider in Settings → AI");
