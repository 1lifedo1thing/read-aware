...
function referenceParameters(scope: ThreadScope, state?: AgentTurnState) {
  return Type.Object(withSpoilerArgument(scope, { reference: Type.Object({ bookId: Type.String({ minLength: 1, maxLength: 512 }),
    ...
    offset: Type.Optional(Type.Integer({ minimum: 0 })), limit: Type.Optional(Type.Integer({ minimum: 2, maximum: 12000 })) }, state), { additionalProperties: false });
}
...
export function buildReferenceTools(scope: ThreadScope, deps: RuntimeDeps, state?: AgentTurnState): AgentTool[] {
  ...
  return [{
    ...
    parameters: referenceParameters(scope, state),
    execute: async (_id, params, signal) => {
