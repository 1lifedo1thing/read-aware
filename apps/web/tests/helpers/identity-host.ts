...
export function identityHost() {
  ...
  const service = createIdentityConsolidationService({
    ...
    mint: async drafts => {
      ...
      return drafts.map(draft => ({ ...draft, origin: draft.origin === undefined ? undefined : actorOrigin(draft.origin), id: `event-${++counter}`, hlc: { wallMs: 1, counter, deviceId: "test" },
    ...
    invoke: async <T>(command: string, args?: unknown): Promise<T> => {
      ...
      if (command === "identity_work_read") { await controls.beforeRead(); return { revision: identityRevision, index: 0, pageCount: 0, baseIndex: 0, checkpoint: null, json: null } as T; }
      if (command === "identity_work_compact") { await controls.beforeCommit(); return { revision: identityRevision, pageCount: 1, status: "compacted" } as T; }
