...
export default {
  activate(ctx) {
    ...
    ctx.contributions.commands.register({ id: "inspect", title: "Inspect authorized panels", run: async () => ({ toast: JSON.stringify({
      hasReader: !!ctx.services.ui.reader, hasWrite: !!ctx.services.ui.reader?.setPanel, hasFocus: !!ctx.services.ui.reader?.focus,
      snapshot: await ctx.services.ui.reader?.snapshot(), seen,
    }) }) });
    if (ctx.services.ui.reader?.focus) for (const target of ["content", "toc", "chat"] as const) {
      ctx.contributions.commands.register({ id: `focus-${target}`, title: `Focus ${target}`, run: async () => {
        const before = await ctx.services.ui.reader!.snapshot();
        if (!before) throw Error("No ready panels");
        const receipt = await ctx.services.ui.reader!.focus!(target, { sessionId: before.sessionId, bookId: before.bookId });
