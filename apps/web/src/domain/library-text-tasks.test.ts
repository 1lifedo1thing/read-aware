...
const task: BookTextTaskSnapshot = { bookId: "book", taskId: "task", mode: "prepare", status: "paused", revision: 2,
...
function plugin(permission: PluginPermission) {
  const runtime = buildPluginContext({ id: "text-control", name: "Text", version: "1.0.0", schemaVersion: 1,
    permissions: [permission], requires: { domains: { library: "^1.21.0" } } }, "0.5.4", []);
