...
export function createReadingDomain(origin: DomainActor, lifetime?: AbortSignal, trackCleanup?: (work: Promise<void>) => void, owners: DomainActorOwners = {}): ReadingDomain {
  ...
  const commands: ReadingCommands = {
    ...
    configureMode: (input, signal, guard) => readingRuntime.configureMode(input, signal, guard, origin),
    returnToMode: (signal, guard) => readingRuntime.returnToMode(signal, guard, origin),
    stepMode: (direction, signal, guard) => readingRuntime.stepMode(direction, signal, guard, origin),
    ...
    goTo: (target, signal) => readingRuntime.navigate(target, signal, origin),
    back: (signal, guard) => readingRuntime.back(signal, guard, origin),
    forward: (signal, guard) => readingRuntime.forward(signal, guard, origin),
    step: (direction, signal, guard) => readingRuntime.step(direction, signal, guard, origin),
    reload: (signal, guard) => readingRuntime.reload(signal, guard, origin),
