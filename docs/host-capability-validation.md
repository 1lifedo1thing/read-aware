...
| TXT12 | 书内图片读取与灯箱缩放预览 | F2 | 本机FB2图像/灯箱及Text Desk真实远端识图通过（精细几何有误）；Agent发现错ID/复验超时失败；其他格式待验 |
...
| SYS02 | 插件私有文档 CRUD/搜索/分页/条件事务 | F8 | 部分通过：真实Worker文档写入重读保留普通回调样式字段、清理数量0；搜索/分页/条件事务待验，见第十九流程 |
