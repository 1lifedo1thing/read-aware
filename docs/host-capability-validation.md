...
| EXT12 | 安装/授权/启停/更新/回滚/卸载插件 | F7 | 部分通过：正常安装入口真实Worker迁移失败回滚代码/KV/文档、成功升级/降级/卸载（20），现有内置插件停用/恢复（56）；安装同意UI、故障恢复失败/进程中断与发布包待验 |
