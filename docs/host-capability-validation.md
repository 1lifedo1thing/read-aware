...
| LIB08 | 查询/读取书籍原文件与本地可用性 | F1 | 本机 Library Desk 原文件本地状态与备份字节一致通过；导出选择器等待验 |
| LIB09 | 提取/显示封面与封面可用状态 | F1 | 本机 FB2 封面可用/解码及插件副本重启重读通过；锁屏视觉/其他格式待验 |
...
| MEM04 | 记忆巩固、修订/替代/遗忘 | F5 | 本机手动条件遗忘及同文重提抑制通过；模型巩固/替代待验 |
...
| OPS08 | 备份导出与合并导入 | F8 | 本机新隔离库加密导出/错密码拒绝/解密预检含插件资产通过；旧库缺源拒绝；应用恢复等边界待验 |
