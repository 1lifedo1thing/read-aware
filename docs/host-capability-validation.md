...
| AI02 | 创建/切换/清空全局线程和书内聊天 | F4 | 本机全局新建草稿/消息落库后列出、切换恢复，等待时切走取消且不污染另一会话；删除等待中线程取消并清空，删除末项回到新草稿，通过（60）。修复全局空白页误用本书文案，中英文实际呈现通过；书内独立清空/Agent批准、故障、进程重启、packaged和跨设备待验 |
