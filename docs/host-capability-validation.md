...
| LIB11 | 重复检测、同源书合并和 ID 重定向 | F1 | 部分通过：同文件导入保留ID（9）；真实Library Desk三本同hash组预览/未确认拒绝/过期拒绝/确认合并，笔记/时长/收藏/集合及旧ID解析、删除后源释放通过（57）。Agent合并批准、聊天/记忆/摘要/进度、故障/重启及真实同步/其他格式待验 |
...
| AI01 | 读取书内/全局对话及搜索历史 | F4 | 部分通过：真实书内会话保存/关书重开历史保留（49、51）；全局线程、历史搜索及分页待验，合成消息不计模型质量 |
...
| AI04 | 提问、选项澄清、批准/拒绝高风险动作 | F4 | 部分通过：实际Agent工具经ChatInteractionPrompt拒绝/批准两书删除并核对原生结果（24），不调用模型；完整提问/选项澄清/过期与模型选择动作待验 |
...
| CON02 | 对象级授权/用户批准/来源与审计 | F7 | 部分通过：真实设置混合越权整批拒绝（6）、阅读控制权限/旧工作区revision拒绝（23）、Agent批准与拒绝删除（24）；所有对象/来源审计及跨设备待验 |
