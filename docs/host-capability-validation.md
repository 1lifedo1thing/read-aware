...
| OPS02 | 事件/Blob E2E 加解密、游标、去重/确认与重试 | F8 | 部分：第83双端密文事件/Blob解密、游标去重ACK及故障恢复收敛；重启/换代失败/物理跨设备待验 |
...
| CON01 | 能力发现/版本/权限/依赖与安装同意 | F7 | 第80流程发布包Library Desk安装同意显示版本/作者、四类权限及六项设置读写路径，Install后已启用且真实Worker执行、确认卸载恢复原集合；能力目录分页/版本依赖拒绝等仍待验，已有证据对账见第81项报告 |
