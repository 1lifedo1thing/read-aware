...
## 机器通过抽查补充：五本真书的五个未审样本

桌面再次锁定后，插件复合过滤停在打开文件面板之前，Library Desk已关闭，
没有新导入或资源lease；已告知用户待解锁。转而预先选五条此前未人工评分的
固定首轮机器通过记录，复用问题、完整回答、工具轨迹并对照相关原文，不再跑模型。
按原有四维和4–5通过/3有保留/1–2不满意判据，得到2通过、2有保留、1不满意：

| 场景 | 总评 | 产品判断 |
| --- | --- | --- |
| S07.14 · 多轮代词承接 | 3 | 正确承接米嘉并回答生母命运；首轮章索引误标章号，背景过渡句未展开，末轮传记过长。 |
| S15.28 · 引文说话者与场景 | 3 | 章节、叙述者及场景有据；主动复述大量剧情，超出定位需要。 |
| S08.1 · 说服手段关系 | 4 | 三者关系和例子对应原文，帮助理解；个别绝对说法和末尾延伸可收敛。 |
| S02.32 · 开放式问题在哪里 | 2 | 主题标题和用法有据，但索引冒充章号，并凭位置编造已读/未读经历、过度解读未命中。 |
| S14.5 · 长函数与嵌套条件诊断 | 4 | 具体手法和使用条件有原文支撑，没有一概要求多态；备选手法展开偏多。 |

五条总体/四维/标签/具体评语已写入各原run的私有human-reviews.json；逐项确认
旧评分未变、原模型输出hash未变。累计首轮机器通过抽查由14增至19，不能据此
估算全部198条质量；247/198/34/15固定统计保持。原文与模型全文不提交。
...
固定选取此前未人工评分的S02.34和S08.35，读取完整问题、工具轨迹、回答和
状态；未调用模型或改写原输出。两条都按原四维判为3分/有保留：高亮和笔记
正文正确，但笔记仅传quotedText而无range/anchor，回答仍声称已附着在原文。
S02.34还把不含问号的查询范围与包含问号的exact/text混用；原内存端口未拒绝。

代码核对确认验证缺口：fixture的readRange没有比较textQuote，标注写入丢弃
range，场景state仅保留kind/text。现有机器通过因此不能证明真实宿主的完整
范围校验、持久锚点或绘制。后续需修复评测端口并定向验收，不把这次历史审阅
