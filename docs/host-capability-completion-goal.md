...
| C04 | 跨插件事件因果防环 | 跨领域写、Worker 回调及派生任务传递宿主签发的因果链；循环自动动作被明确拒绝，独立用户操作与合法后续任务不被误伤 | 实施中，事件/Worker、设置/文档/标注/记忆快照及投影通知已接，图谱任务和正文准备来源已接；阅读器开书/控件/播放/显式选区、导航、模式及面板开关/宽度主链已接，设置重排、原生派生反馈及其他观察来源仍缺 |
