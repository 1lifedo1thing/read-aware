...
| C08 | 统一长任务恢复 | 类型化多步骤工作有持久任务身份、进度、取消/暂停/恢复、幂等回执及跨重启续跑；接入现有任务消费者，失联结果必须先查证而非盲重放 | 实施中：类型化计划、schema47检查点/修订围栏和宿主恢复调度底座已写；暂停/取消意图先持久化，未知结果保留核对；事务/正文/图谱步骤已接统一宿主执行器，Agent/插件及消费者尚未接通，真实重启验收待执行 |
...
- C08 当前底座：不可变计划与步骤派发身份、结果回执、进度、控制意图以 owner 隔离写入 SQLite；
  ...
  仍无公共可用调用链；下一步接生命周期恢复、Agent/插件授权入口与现有消费者，
