...
| C04 | 跨插件事件因果防环 | 跨领域写、Worker 回调及派生任务传递宿主签发的因果链；循环自动动作被明确拒绝，独立用户操作与合法后续任务不被误伤 | 实施中，事件/Worker及导入图谱写已接；观察来源仍缺 |
...
- C04 下一步：接快照观察、合并/重读观察的因果来源（不能漏标后当独立根），
  以及阅读器异步生命周期反馈、插件文档变更等仍未保留来源的派生路径；观察者
  自动写的第一方消费与跨插件组合补验。当前仅 subscribe 公共链已接，不能计整项
  完成。C05–C09仍未实施，C01–C03真实桌面组合验收仍待集中执行。
