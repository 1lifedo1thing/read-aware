...
| C05 | 完整操作可用性 | Agent 与插件能查询操作需要的权限、阅读对象、账号、模型/端点和提供者条件；区分未配置、不可用、未知；执行前重验，不把注册或网络提示当健康 | 实施中：推理、书域朗读/模式及正文准备条件查询、实际执行重验及Text Desk消费者已接；library1.31正文任务接当前书授权的准入/执行围栏；sync.now连接/凭据/提供者条件及Maintenance Desk消费者已接；其余操作条件仍缺，集中桌面验收待执行 |
