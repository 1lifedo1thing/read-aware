...
| C07 | 跨领域提交与统一撤销 | 多个可事务化本地语义操作经冻结预览/版本校验在同一提交中全成全败，含设置、私有文档及领域变更；Agent/插件共用，撤销是新的受控条件提交；非事务性外部副作用在预览时明确拒绝纳入原子批次 | 实施中：原生同事务事件/设置/文档及条件检查已写；SQLite冲突回滚及成功提交已验证；语义预览、授权、条件撤销、Agent/插件及消费者未接 |
