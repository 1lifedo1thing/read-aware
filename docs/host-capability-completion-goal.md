...
storage2.6为公开storage.onChange接入稳定ruleId、反应凭证与withEvent；宿主沿既有设置通知保留来源，异步回调串行并合并等待变化，订阅不主动首发，释放后停止后续通知。Worker代理传递回调和规则选项。受影响Web/desktop类型、公开上下文防环/串行/释放及Worker回调资源释放检查通过；合跑Worker套件另有两项图谱退出检查超时，原因未定位，不计通过。尚无第一方插件实际消费此入口，真实Worker/Tauri未验；C04/C05其余断点和C06–C09集中桌面验收仍待完成。下一步优先接剩余实际调用链，不为无消费者的入口添加演示功能。

## 本批传输待发布来源交付

同步传输注册表同批来源合并已补：每个entry的注册/失效/退休待发布来源以有界merge累积，发布后清空；失败候选回滚仍删除自身待发布来源，不污染恢复的原entry。既有会话检查扩验两次失效来源均保留、嵌套替换失败来源排除、最终退休来源独立，释放/迟到打开/缓存失效检查通过，受影响类型通过。现有调度和公开同步观察沿注册通知消费；真实Worker/Tauri及其余C04/C05与完整目标待完成。
...
诊断报告准备链已接：宿主请求进入hook后保留因果来源至assembling/preview和bundle组装，组装传入底层共享投影校验；确认发送/导出捕获新的用户来源，working、sent/失败重试预览和流程回执同源。来源为WeakMap元数据，不序列化到报告正文。受影响类型及既有隔离React预览/确认/取消迟到回执检查通过；未发送真实报告。报告网络/导出原生验收、其他C04/C05及完整目标仍未完成。

## 本批诊断流程来源交付

诊断来源已接：插件报告/修复请求带来源进入导航；verifyProjectionReport在首次物理派发时捕获来源，设置页与插件加入同一校验保留首发来源，聚合摘要和各等待者副本保留来源且不泄露原始样本。投影修复检查/预览沿请求来源，宿主确认后工作/完成与流程回执共享新的用户来源。既有共享取消、摘要隔离、修复确认/释放检查与受影响类型通过。报告内容准备/发送状态、修复原生跨重启来源及真实Tauri仍待验，C04/C05和完整目标未完成。

## 本批更新检查可用性交付

session2.12新增maintenance.checkForUpdates条件查询，Agent工具同样可查；插件先检查service:network，未授权不读取更新器状态。宿主共用执行门槛：不支持/安装中拒绝，正在检查可加入共享执行，服务器健康保持unknown。Maintenance Desk0.10新增条件页与明确检查动作，支持刷新，未知允许尝试；执行仍重验锁。受影响Web/Agent/插件类型、构建及权限/共享检查/安装锁/实际编译消费者检查通过，生成文档通过。真实原生更新与其他C04/C05及完整组合验收仍待完成。

## 本批全量恢复宿主来源交付

全量恢复确认来源已传入隔离迁移Worker启动/终止、review.apply队列、恢复回执和恢复后插件停用；来源在异步受理前捕获，移除taskId时保留回执来源。shutdownPlugins将同一来源交给各插件停用。保留提交/恢复失败后直到reload的写入暂停和已受理操作结算约束。受影响类型与既有review单次消费、取消/释放、程序审查检查通过。原生跨重启历史来源、其他备份UI状态和真实Tauri仍待验，C04/C05及完整目标未完成。

## 本批旧版备份写入来源交付

旧版v1书库导入已在文件读取/排空前捕获actor，传入KV、资料、书籍/集合和批注恢复；资料广播与投影失效通知沿该来源，批注成功写入也发布恢复失效。保留原非原子合并、已受理写入排空及文件引用释放。受影响类型及现有资料冲突、备份隔离/取消/文件释放检查通过。全量加密恢复、备份模态状态、跨重启genesis来源与真实Tauri未验，其他C04/C05及完整目标仍未完成。

## 本批维护入口来源交付

维护入口来源已接：插件requestConnectionTest/requestBackup/openSettings将调用actor传到宿主流程和workspace导航；原生AI/备份入口定位复用已标记请求，openSettings导航、控件定位与回执共享来源。复用HostActionFlow的确认/取消约束。受影响完整类型及既有维护导航、流程取消和投影修复检查通过；备份内部持久写、AI原生调用状态、其他模态来源与真实Tauri仍未闭合，C04/C05及完整目标继续。

## 本批公开连接流程来源交付

公开同步连接流程已接来源：插件打开设置/请求流程沿调用actor进入导航和宿主请求；账单自动交接沿请求来源，原生用户确认建立新的用户来源并贯穿连接操作、凭据/重启和完成回执。共享HostActionFlow保留确认前取消、确认后等待实际操作结算与账号换代校验。受影响类型及既有流程/投影修复/连接排他检查通过；未执行真实登录、删除账号、账单跳转或Tauri验收。其他宿主流程/模态状态来源、C04其余来源与C05条件余项继续保留，完整目标未完成。

## 本批连接操作来源交付

宿主确认后的连接操作已贯穿来源：runSyncConnectionOperation将捕获actor交给实际操作，连接/断开/删除账号后的本地断开、传输后端连接复用至秘密写入、提示清理和调度重启；新调度的初始化/首轮状态沿连接来源，后端重新注册触发沿注册通知。既有模拟IPC备份排空/连接持久化和失败删除检查补验忙碌/重启来源通过，类型通过。未执行真实登录、远端删除或跨设备同步；公开连接流程导航/回执来源及其他C04/C05边界仍需接续，目标未完成。

## 本批封面后台补齐来源交付

封面后台补齐已接周期→下载→book-changed来源：重试保留缺失项来源，并发触发合并下一轮来源；fetchRemoteBlob在排队前捕获来源，备份自有下载也可传递来源。停止/账号换代使旧列表或下载回执失效，不再发布旧完成或重建旧重试，新请求等待旧物理执行结算后继续。既有模拟IPC同步/备份场景补验下载来源、停止后迟到结果与新代恢复通过，类型通过；不声称强制取消已派发下载。沿现有Library Desk富化观察消费书籍通知，真实Worker/Tauri跨设备、连接流程和后端重注册等来源及剩余C04/C05仍待完成。

## 本批同步周期观察交付

sync1.2周期状态观察已接：手动请求在异步校验前捕获actor，同一物理周期的进度/完成/失败沿来源，串行引擎进度接收器在结算时移除；本地事件触发的推送合并来源，同步后书架/会话/漫游刷新沿周期来源。连接busy开始/结束共享操作来源。公开观察保留慢回调合并与失败来源，丢过期采样并在释放后停交付；Maintenance Desk0.9使用稳定sync-live反应刷新。受影响类型/构建、控制器/观察与显式SYNC_BACKUP_PROOF模拟IPC调度运行通过；真实Worker/Tauri跨设备、连接流程来源、封面补齐/远程读取/后端重注册与剩余C04/C05仍未闭合。

## 本批更新观察交付

maintenance1.5更新观察已接：共享检查沿受理来源、通道换代合并检查期间来源，安装进度和安装器打开保留宿主操作来源；维护观察沿原子状态/通道通知串行合并、失败续接、退休释放。Maintenance Desk0.8更新页使用稳定updates-live反应自动发布并跳过cycle，安装权限仍仅宿主用户操作。受影响类型/构建、既有共享检查取消/通道过期/观察释放及消费者定向检查通过；真实Worker/Tauri原生更新/安装仍待集中验收。同步状态与连接流程、其余C04/C05和完整目标未完成。

## 本批计划注册与退休交付

计划注册/退休已接现有贡献句柄：注册捕获来源，显式/反应绑定释放及生命周期退休向调度器传递来源。具名计划规则由控制器的ID/代际约束管理，保留句柄身份，释放后重绑和活跃同名替换不再被观察规则名占用；旧句柄不移除新绑定。实际插件上下文观察→反应释放→cycle、释放后重绑/旧句柄释放及调度检查通过。真实Worker/Tauri启停/替换仍待集中验收；接续同步/更新观察和C05余项，总目标未完成。

## 本批计划执行与观察交付

schedules2.1计划执行与观察已接来源：手动控制/排队/取消及状态KV写保留actor，周期触发为独立系统来源，延迟排队持久保存有界来源并在同版本恢复时续接，公共状态不泄露来源字段。具名bind回调签发稳定反应凭证，执行完成/失败沿回调反应，observe串行合并、去重和失败续接；RSS0.23定时刷新及计划状态页通过withEvent消费。受影响类型/构建、既有调度/持久写/退休检查及实际插件上下文await后控制/凭证过期、模拟重启来源保留通过。真实Worker/Tauri/RSS网络/原生重启仍待集中验收，注册退休的完整来源及同步/更新观察、C05余项保持未完成。

## 本批导入任务观察交付

library1.35导入任务观察已接：启动在异步受理前捕获来源，状态/查询副本保留来源，取消合并控制来源，串行观察保留合并和失败来源；公开反应凭证/稳定ruleId已接。Library Desk0.17确认导入后转到任务页，显示阶段/失败/最终回执，受理前可取消，自动更新沿withEvent并跳过cycle；文件引用由确认页转交任务页，关闭后释放。受影响类型/构建、既有取消/持久写租约排空及消费者检查通过。真实Worker/Tauri导入仍待集中验收；计划、同步/更新观察和C05余项继续保留，总目标未完成。

## 本批正文来源观察交付

library1.34正文来源观察已接虚拟书失效调用来源、提供者贡献变更、绑定KV变化及书籍事件；串行等待持久写，过期查询重取、失败续接、退休释放。公开observeContentState支持反应凭证/稳定ruleId，Text Desk0.34新增正文来源页，经withEvent更新本地文件/提供者可用状态。实际插件上下文await→自有失效→cycle、受影响类型/构建及消费者释放检查通过；查询不加载正文或访问网络。真实Worker/Tauri与原生文件变化覆盖仍待集中验收；导入任务、计划、同步/更新观察和C05余项未完成，完整目标保持。

## 本批富化任务观察交付

library1.33富化观察链已接：共享队列排队/执行/终态保留请求来源，排队扩展合并双方来源；书籍事件与持久写结算进入串行观察，过期读取重取，失败保留来源，退休释放订阅。公开observeEnrichment提供反应凭证和稳定ruleId，Library Desk0.16封面/元数据页用withEvent刷新并跳过cycle。受影响类型、插件构建及既有队列/权限/错误恢复/释放与消费者检查通过；真实Worker/Tauri提取及SQLite验收仍待集中执行。其余正文内容状态、导入任务、计划、同步/更新观察与C05余项继续保留，完整目标未完成。

## 本批正文任务观察交付

library1.32正文任务观察链已补：状态/查询副本保存宿主来源，暂停和取消沿明确控制actor，优先级从Agent/插件命令传入来源；历史pending/成功/失败通知沿实际记录的来源，过期历史回执不替换新状态。初次观察沿订阅来源，慢回调合并来源、失败交付保留到下次；公开反应凭证与稳定ruleId在书域过滤后签发。Text Desk0.33任务页用withEvent自动发布并跳过cycle，动作保留用户上下文。受影响类型、构建、既有任务控制/书域/观察/历史失败恢复和插件检查通过。真实Worker/Tauri正文准备/SQLite及其余富化、计划、同步/更新观察仍待完成；完整C04/C05与总目标未完成。

## 本批公开会话观察交付

conversations1.6已接会话运行态公开观察：宿主串行合并变化/失败重试并限制64观察，初次沿订阅来源，生命周期释放；书域先过滤全局线程和未授权会话，再按授权投影版本交付，阅读器变化与运行态变化串行合并。公开反应凭证支持稳定ruleId，Memory Desk0.18实时控制页用withEvent发布并跳过cycle，点击保留独立上下文。受影响Web/插件类型、构建、权限/书域及编译消费者检查通过；同进程真实插件上下文→await→stop→反馈cycle和凭证过期已验。下一步继续正文/富化、计划任务、同步/更新观察及消费者；C04其他来源、C05余项和真实Worker/Tauri/模型验收仍待完成，完整目标保持未完成。

## 本批聊天状态来源交付

useBookConversation已接来源：消息/加载/流式状态分别带来源，按实际变化字段发布运行态；打开沿当前全局选择或阅读器来源，无法归因时保持系统来源。用户发送/重试捕获同一回合actor，停止将明确控制来源交给在途回合，部分回复持久写和最终状态保留合并来源，消息事件仍保留user/agent角色。同步刷新在途/失败保留通知来源、过期查询不写回，延迟刷新使用当前绑定；旧回合完成不污染新会话的消息或状态。受影响类型、原有请求/清空/权限/备份持久写边界及临时真实React Hook的停止→部分保存→运行态/切书检查通过，未新增仓库测试文件。旧备份用例等待已废弃ai_chat_clear已对齐实际commit_events清空事务。下一步继续串行公开observeRuntime、书域过滤与Memory Desk消费者；实际模型/Worker/Tauri仍待集中验收，完整C04/C05和总目标未完成。

## 本批会话来源基础交付

会话来源基础已补：全局线程选择在异步持久写前捕获actor，内存ID保持原接口并另存来源，漫游覆盖沿通知来源；运行态changed通知携带独立宿主来源，绑定/更新/退休可显式传入来源，重入通知不覆盖先前来源。回合请求保留actor及原owner隔离，取消/过期沿请求或明确取消来源，用户采用/拒绝为用户操作；停止/清空在校验前捕获同一actor并用于取消、排空及持久写/广播。受影响类型与现有权限/请求退休/最终持久化排空检查通过。下一步必须继续useBookConversation加载、发送/流式/停止与延迟同步来源，再接串行公开observeRuntime和Memory Desk；这些尚未接通，不把基础元数据当完整防环。计划/正文/富化/同步/更新观察、C05余项及真实Tauri仍待完成。

## 本批插件目录观察交付

plugins1.10安装状态目录观察已接来源：初次沿订阅，后续沿installedPluginsAtom；串行合并慢回调、保留失败交付来源、64观察上限与退订释放。公开目录/贡献观察支持稳定ruleId，Maintenance Desk0.7目录页和贡献页通过共享liveView消费withEvent并跳过cycle，用户动作保持独立上下文。受影响Web/插件类型、插件构建、现有宿主目录与消费者检查通过。下一步继续会话运行态、正文/富化任务、计划任务及同步/更新观察的来源和实际消费者；新目录链真实Worker/Tauri待集中验收，完整C04/C05及总目标仍未完成。

## 本批插件窗口观察交付

UI1.17窗口观察已接公开反应凭证/稳定ruleId及Workspace Profiles0.10实际窗口页。控制通知带来源，观察合并慢回调期间的通知，舍弃过期读取时保留来源，错误和回调失败可继续沿来源恢复；初次观察沿订阅来源，原生轮询沿实际快照来源而非新根。自动视图发布用withEvent并跳过cycle，点击保留独立操作。受影响Web/插件类型、插件构建及窗口控制器/编译消费者检查通过，沿用既有用例补来源断言；没有以版本号常量代替功能验收。原生窗口输入/动画来源现有限制及真实Worker/Tauri反馈链仍待验，完整C04其他来源、C05余项和总目标未完成。

## 本批插件工作区观察交付

UI1.16已将workspace/commands观察接入公开插件反应凭证和稳定ruleId：授权投影后签发，异步回调完成后失效，null退休也有独立交付来源。受限工作区把宿主和阅读器变化合并为串行观察，初次阅读器通知沿订阅来源，后续沿真实阅读器来源；释放全部订阅。Library Desk0.15两个实时视图通过withEvent发布并跳过cycle，点击仍使用独立上下文。受影响Web/插件类型、插件构建、宿主/书域及消费者检查通过；同进程真实插件上下文→workspace导航的await/凭证过期/反馈cycle/null退休链已验证。新公开链的真实Worker/Tauri组合仍待集中验收，不能将同进程检查当桌面证明；完整C04其他来源、C05余项及总目标仍未完成。

## 本批宿主工作区观察来源交付

工作区宿主观察已改用来源独立于值的串行快照交付：null关闭事件可带退休来源，绑定/变化发布沿实际来源，初次观察沿订阅来源，慢回调期间合并变化，失败重试保留来源。命令观察沿工作区、设置、localeActor来源，丢弃过期读取时不丢来源，失败状态及回调失败保留到后续交付；语言变化不再接受字符串作事件来源。受影响类型与既有工作区/命令/书域检查通过，既有用例增加null退休来源及错误恢复来源断言。此批完成宿主观察基础，公开插件workspace/commands回调反应凭证、稳定ruleId、书域阅读器变化来源及实际消费者仍未接通，下一步必须继续这些调用链；真实Tauri及完整C04/C05和总目标仍未完成。

## 本批书架整理来源交付

书架后台整理已沿来源接到工作区：书籍/集合内存投影保留宿主来源，library-changed/book-changed/book-removed刷新沿触发事件；集合失效和选择清理按变化输入传递来源，不再默认成用户操作。异步刷新串行，读取中有新通知则丢弃旧样本并合并来源重读，失败保留来源等待下一次实际刷新，卸载后停止发布。受影响类型检查及临时React检查覆盖读取中删除、旧样本不复活书籍、来源保留和卸载；未新增仓库测试文件。真实Tauri及完整观察防环仍待集中验收，下一步继续null工作区快照、workspace/commands公开观察反应凭证及实际插件消费者；其余C04/C05与总目标未完成。

## 本批设置栏目来源交付

SettingsDialog已把深链请求来源保留到实际栏目写入，消费请求时的清空同源；手动切栏/返回分别建立用户来源，打开重置沿打开来源，插件条目移除或重排改变当前栏目时沿注册表来源。只对实际栏目变化发布来源，视口变化作为系统事件，不把自动派生误标为用户操作。受影响Web/Foliate/desktop类型检查通过；未新增测试，真实React/Tauri栏目切换与防环组合仍待集中验收。下一步继续书架后台整理来源、null工作区快照和workspace/commands公开观察反应凭证及消费者；完整C04/C05与总目标保持未完成。

## 本批工作区状态来源交付

C04工作区状态来源已从写入口接到实际快照：临时UI atom保留原值/函数更新用法，宿主专用来源与值分离；工作区适配器同批导航写共享已捕获actor，搜索打开引起的清空同源。useWorkspaceShell按实际对外变化字段合并来源，不靠请求目标匹配猜测；未变化字段不混入新根，阅读器切入/退出沿readingRuntime快照。WorkspaceService绑定/发布/分页查询及受限投影保留宿主来源，重复同值发布保留原来源。受影响类型与现有工作区/适配器/书域、来源投影检查通过。下一步必须继续同链：SettingsDialog请求→实际section派生来源、书架后台整理来源、null快照与公开workspace/commands观察反应凭证和实际消费者；这些尚未接通，真实React/Tauri因果组合仍待验，其余C04/C05范围不变。

## 本批宿主命令条件交付

session2.11新增ui.commands.execute(command)条件查询：与真实命令共用权限/阅读控制、工作区版本及选择容量检查；查询不保存设置、不导航，目标加载/提交成功保持unknown。受限插件先验书范围并在异步查询前后复核，使用自己的工作区版本转换；Agent和Library Desk0.14动作已消费，明确不可用显示错误/刷新且不派发，执行再次检查版本。设置已保存但导航失败仍返回partial，不误报回滚或自动重试。受影响Web/Agent/插件类型、插件构建、既有命令/书域/取消/部分回执及消费者定向检查通过。真实Tauri命令组合、工作区快照因果及其他C04/C05仍待完成。
...
C04宿主→Worker镜像发送失败已带来源：本地KV、plugin-storage-changed及locale广播把实际宿主actor传给发送边界，发送失败的贡献退休/错误状态沿同一来源，不重置为独立根。迁移请求在pending回执中保存宿主来源，完成或超时退回activating后发生发送失败仍沿原迁移来源。来源只在宿主闭包/回执中，不放入Worker可伪造消息；首次退休来源不被后续失败替换。受影响类型与定向镜像失败/迁移回执失败及启动来源检查通过；真实Worker/Tauri失败组合仍待集中验收。无法定位具体调用的原生Worker错误、其余C04来源及C05余项保持未完成边界。

## 本批贡献关闭来源交付

C04关闭来源已贯通停用、改书授权、卸载、更新换代及候选回滚→Sandbox终止→取消/贡献注销。生命周期在取消前只捕获一次宿主来源，重复关闭不换根；注销不再误用旧注册来源。manifest字体/主题已进入同一生命周期的延迟注册及回滚/崩溃清理，注册保留启动来源，关闭保留本次来源；崩溃目录清理与错误状态共用同一失败来源，迁移失败沿已知启动来源，其余无法归因的Worker原生错误仍为独立系统失败，不伪称知道触发调用。受影响类型及既有Worker关闭/换代/持久写排空、manifest资源退休和生命周期回滚检查通过。真实Tauri停用/换代/崩溃及布局反馈待集中验收；C04其余来源、C05余项及总目标仍未完成。

## 本批文件导出条件交付

session2.9新增ui.exportFile条件查询，仅接收filename/byteLength/mimeType，不传正文、不建临时文件、不弹保存框。描述校验与实际导出共享64 MiB及文件元数据边界，宿主执行前按实际内容重验；存在入口仅为unknown，不保证磁盘、保存权限或用户确认。Agent查询/导出工具及Annotation Desk0.9真实导出动作已接，明确不可用不派发，取消保留saved:false，真实写入失败继续上抛。受影响Web/Agent/插件类型与插件构建通过；原有核心边界、Agent/插件取消失败和宿主无副作用/入口变更检查通过。真实Tauri保存/落盘、C04剩余来源和C05其余操作仍待完成。

## 本批剪贴板及外链条件交付

session2.8新增clipboard.writeText(text)及ui.openExternal(url)条件查询，复用实际输入边界和hostIO本地入口检查；插件分别先验service:clipboard/network，拒权只返回权限条件。查询不复制、不打开、不读取剪贴板，返回不回显输入；入口存在仅标unknown，未探测OS权限/默认浏览器。Agent统一条件工具与实际copy/open工具已接，派发前复核，执行入口同源重验；明确不可用拒绝，unknown允许尝试并保留真实失败。受影响类型和定向公共权限/无副作用/入口变化、Agent拒绝派发检查通过；真实Tauri和其余C05仍待完成。

## 本批凭据持久来源交付

schema51已为凭据待发布记录增加source_json。秘密写/删在同一原生事务内存储最新值和最新宿主因果来源；后续本地写替换来源，补发扫描保留已有标记来源，旧/恢复来源未知时不继承旧值来源。沿用有界持久来源校验并对齐宿主UTF16长度限制；发布在同一事务读取值和来源，回执按事件ID返回来源，宿主恢复反应链后广播；来源不进入秘密事件payload，批次容量计入来源。SQLite重开、来源替换/无效来源回滚、删除/补发及受控宿主恢复防环检查通过，受影响类型和原生编译通过。备份前置检查暴露旧版本门禁及schema46–50表策略遗漏：版本对齐51，atomic_receipts/durable_jobs/变化日志及游标明确保留目标设备，不导入源设备的执行意图或回执授权；凭据待删除记录的备份路径已定向检查。旧记录无来源仍保持未知，真实Tauri跨重启与完整恢复链待集中验收；其余C04/C05和目标范围不变。
...
| C05 | 完整操作可用性 | Agent 与插件能查询操作需要的权限、阅读对象、账号、模型/端点和提供者条件；区分未配置、不可用、未知；执行前重验，不把注册或网络提示当健康 | 实施中：推理、书域朗读/模式及正文准备条件查询、实际执行重验及Text Desk消费者已接；library1.31正文任务接当前书授权的准入/执行围栏；sync.now连接/凭据/提供者条件及Maintenance Desk消费者已接；memory.graph.generate任务容量/持久章节边界/fast模型及Memory Desk批准前条件页已接；窗口控制与Workspace Profiles消费者、剪贴板/外链与Agent执行工具、导出描述/入口与Annotation Desk消费者已接；其余操作条件仍缺，集中桌面验收待执行 |
