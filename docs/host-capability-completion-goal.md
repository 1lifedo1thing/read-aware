...
| C04 | 跨插件事件因果防环 | 跨领域写、Worker 回调及派生任务传递宿主签发的因果链；循环自动动作被明确拒绝，独立用户操作与合法后续任务不被误伤 | 实施中，尚无公共接线 |
...
- C04 实施中：新增宿主不可伪造的 DomainActor/EventCause，软件来源仍以原始字符串
  持久化；因果路径在宿主 WeakMap 中传给领域广播、设置变更、KV 提交与阅读状态。
  书库/导入/标注/记忆/会话等写入入口可传递不可变 actor，异步等待不依赖共享全局变量。
  新 PluginEventReactions 管理每激活的事件投递令牌；重复订阅步骤/32层链拒绝，
  令牌不能跨激活或投递结束后使用，已受理异步工作仍保留原因果链。宿主定向测试
  覆盖 A→B→A、并发独立根、条件提交失败不广播、读快照不披露内部路径。
  这仅是 C04 内部实现，未接 Worker 消息、插件公开 API 或第一方消费者，不计接通。
- 下一步优先完成 C04 的公共调用链：事件回调携带宿主令牌，后续领域/服务写入
  在既有权限、对象围栏和生命周期下使用该令牌绑定的 actor；不能另造共享可变
  currentActor 或按“某插件正在处理事件”归因（异步并发会误伤独立用户动作）。
  需要明确传递上下文的 API，并覆盖真实 Worker 和派生任务；不以只添加原因标签
  或只屏蔽自己事件代替防环。图谱任务等未消费 actor 的派生写、合并/重读观察的
  因果来源仍须接通。C05–C09 仍未实施，C01–C03 的真实桌面组合验收仍待集中执行。
