...
- 宿主：实装 207、部分 34、非桌面 1、待建 1。
- Agent：接通 150、部分 41、扩展 11、接通（待 E2E） 15、自动 13、未接 13。
- 插件：接通 165、部分 56、接通（待 E2E） 17、未接 5。
...
| <a id="READ17"></a>READ17 | 列声音并合成音频的提供者 | 实装 | **接通**：get_setting_options 查询 TTS 动态声音；update_settings 选择配置<br>[设计] 受控声源选择 | **接通**：voiceProviders.register/listVoices/synthesize<br>[设计] 语音贡献 | tts；宿主系统语音回退 | 声音配置按既有插件设置选择；settings1.10已接动态选项，不另建voice枚举/任意合成模型工具。提供者启用后仍按宿主既有第一个可用提供者规则，不新增声源优先级选择器。Agent不能借设置接口声称已经触发朗读。声音查询仅成功提升后启动，每注册至多一个在途查询，设置变化合并到最新版本，迟到结果不覆盖新配置；读取失败保留上次有效列表并记录，替换/注销/取消不再刷新。列表只接收复制后的声明数据，所有返回回调在有效/过期/非法/停用分支释放；合成前后核对注册owner与设置版本，拒绝旧提供者及非音频字节，释放非法/过期结果回调。Worker定向消息验证刷新、回调释放和字节返回，未改变既有权限或提供者优先级；未承诺中断已派发远端推理，真实TTS服务商/播放组合待集中验收 | [TTS](../plugins/tts/src/index.ts) [CTX](../apps/web/src/features/plugins/runtime/plugin-context.ts) [API](../packages/plugin-types/src/index.ts) [AUDIO](../apps/web/src/features/reader/hooks/useReadAloud.ts) [DYNAMICOPTIONS](../apps/web/src/domain/settings/dynamic-options.ts) [PLUGINOPTIONSOURCE](../apps/web/src/domain/settings/plugin-option-source.ts) [VOICEPROVIDERLIFECYCLE](../apps/web/src/features/plugins/runtime/plugin-voice-provider.ts) [VOICEPROVIDERTEST](../apps/web/src/features/plugins/runtime/plugin-voice-provider.test.ts) | K03, K05 |
...
| <a id="CON03"></a>CON03 | 生命周期 staging/activate/deactivate 与资源释放 | 部分 | **自动**：runtime invalidation/flush background<br>[设计] 任务/贡献消费生命周期 | **部分**：注册 scope、视图/transport session 回调 lease 与异步 cleanup 排空<br>[设计] 全来源 structured cancellation | 插件启停/升级；Agent 运行时重建 | Worker error/messageerror/failed、启动超时及boot clone失败统一停传输/取消在途/退休注册与回调/排空异步清理和持久写，停止后健康/迁移/提升稳定拒绝，迟到结果不发送；旧代不移除新代注册或状态。视图移除或失效时释放局部回调，未消费/非法/迟到结果释放，旧 Worker 不关闭新实例对话框；停用退休 transport 后排空异步关闭及持久写。共享贡献注册表与sync transport已接同步提升事务：运行期/manifest批次失败恢复同ID旧对象、owner和动作状态，嵌套失败有独立恢复点，已明确释放的旧代不复活；注册通知等批次结束，transport仅成功后退休旧session，失败保留旧session和未完成open。Worker消息定向验证旧命令RPC状态权限恢复，非真实桌面E2E。声音provider另已接单在途/最新设置刷新、停用与结果回调释放、合成前后所有权及设置版本校验。仍缺其他provider结果/在途effect和真实连接全链路；同步注册恢复不回滚已开始的外部副作用，不等同跨进程升级恢复，GAP06/08/14未整体关闭 | [HOST](../apps/web/src/features/plugins/runtime/plugin-host.ts) [CTX](../apps/web/src/features/plugins/runtime/plugin-context.ts) [WIRE](../apps/web/src/features/plugins/runtime/plugin-worker-host.ts) [CALLBACKWIRE](../apps/web/src/features/plugins/runtime/plugin-callback-wire.ts) [LIFECYCLE](../apps/web/src/features/plugins/runtime/plugin-lifecycle.ts) [CONTRIBUTIONACTIVATION](../apps/web/src/features/plugins/state/contribution-activation.ts) [CONTRIBUTIONACTIVATIONTEST](../apps/web/src/features/plugins/runtime/plugin-contribution-activation.test.ts) [THREAD](../packages/agent/src/runtime/thread.ts) [SYNCSESSION](../apps/web/src/platform/sync/transport-session.ts) [VIEWSESSION](../apps/web/src/features/plugins/lib/plugin-view-session.ts) [VIEWSOURCE](../apps/web/src/features/plugins/hooks/usePluginViewSource.ts) | E04, K06, N05, Q06, R01, R03 |
| <a id="CON04"></a>CON04 | 跨 Worker RPC 的类型、错误与资源额度 | 实装 | **扩展**：插件 tool 也经过同一 worker bridge<br>[设计] 工具任务不被悬挂 | **接通（待 E2E）**：双向准入/握手、单实例及全App存活资源和流量预算<br>[设计] 有界可取消版本化 RPC | 所有 Worker 插件及其 Agent 工具 | __fn/__disposable保持普通数据。双向全部envelope发送前/接收后验证，boot共用manifest校验；boot/hello/ready绑定协议，import/activate前握手，版本/重复启动/非法镜像失败关闭。每条80MiB/100万项/128层，完整buffer与循环别名核算；每消息10万、每激活20万callbacks及4096注册预留。全App新增1024进入/1024回调RPC、16384注册预留、50万存活回调租约；业务前预留，失败不替换贡献，结果/真实注销释放，同ID换代不能清其他实例。双向共用单实例与全App消息/字节/图项token桶，非法消息先计envelope，遍历失败也计已耗额度；全App突发65536条/512MiB/1600万项，秒恢复16384条/128MiB/400万项；实例重启不重置总桶。累计超限退休触发实例并排空，不循环拒绝回包或保留旧镜像。普通单消息拒绝仍可恢复，错误最多4096字符。不是OS消息队列、克隆峰值、插件CPU/堆硬限制或公平带宽保证。完整契约门禁通过；隔离macOS debug真实双Worker洪泛故障中违规实例退休/贡献消失/额度释放，另一实例ping成功，清理回基线。完整崩溃撤权组合、长期压力与packaged跨平台仍待验，GAP07/12/13/17不整体关闭 | [WORKER](../apps/web/src/features/plugins/runtime/plugin-sandbox.worker.ts) [WIRE](../apps/web/src/features/plugins/runtime/plugin-worker-host.ts) [CALLBACKWIRE](../apps/web/src/features/plugins/runtime/plugin-callback-wire.ts) [API](../packages/plugin-types/src/index.ts) [ERRORS](../packages/core/src/errors.ts) [WORKERPROTOCOL](../apps/web/src/features/plugins/runtime/plugin-worker-protocol.ts) [WORKERPROTOCOLPROOF](../apps/web/src/features/plugins/runtime/plugin-worker-protocol.test.ts) [HOSTPROTOCOL](../apps/web/src/features/plugins/runtime/plugin-host-protocol.ts) [HOSTPROTOCOLPROOF](../apps/web/src/features/plugins/runtime/plugin-host-protocol.test.ts) [WIREBUDGET](../apps/web/src/features/plugins/runtime/plugin-wire-budget.ts) [CALLBACKBUDGET](../apps/web/src/features/plugins/runtime/plugin-callback-budget.ts) [HOSTBUDGET](../apps/web/src/features/plugins/runtime/plugin-host-budget.ts) [HOSTBUDGETPROOF](../apps/web/src/features/plugins/runtime/plugin-host-budget.test.ts) [HOSTBUDGETDESKTOP](../apps/web/tests/desktop/desktop-traffic-budget-probe.ts) | J12 |
...
| ID | 宿主能力 | 宿主现状 | Agent 当前与目标 | 插件当前与目标 | 实际消费者 | 缺口/边界 | 来源 | 旧基线 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| <a id="MORE01"></a>MORE01 | 周期调度/启动补跑/失败记录 | 实装 | **接通**：list_plugin_schedules/manage_plugin_schedule 全局<br>[设计] 枚举与批准 pause/resume/run | **接通**：schedules 2.0 bind/list/observe/control<br>[设计] 自有计划状态与受控执行 | RSS 现有每小时刷新；Agent 与插件正式入口 | 每插件最多64个绑定，全App最多1024个，回调每插件2/全App8并行并轮转准入，查询偏移分页 1–100/默认 50，观察初始+串行合并变化。暂停/恢复等持久写；手动 run 越过暂停/间隔一次，不恢复自动计划。每 key 跨重绑定共享 flight，already-running 不假称完成；开始记录落库后才派发 callback，结束记录落库后才 completed，失败不会刷新 lastSuccessAt。旧时间戳只迁为 lastStartedAt；未完成记录显示 interrupted，不伪造成功。新/旧调度记录排除偏好漫游；不是完整执行历史。退休停止新调用与迟到结果写，已派发持久写参与生命周期 drain；不保证物理副作用回滚。每秒扫描，周期任务至少 15 分钟间隔，错过多轮合并一次，关 App 不运行；最后绑定释放计时器。定向持久/并发/权限测试已验，组合插件与 Tauri 重启/升级验收待集中进行。 RSS0.13已迁Schedules2.0并组合自动刷新实时页、暂停/恢复/立即运行，使用Schedules2.0/UI1.2和既有全局Agent工具；缺绑定移除操作，离开退订，重复运行不假报完成。修正计划回调吞掉部分/全部刷新失败：四并发批次完成后抛首个捕获错误，宿主不推进lastSuccessAt；普通Refresh all仍汇总部分成功，旧记录不改。插件27项基础测试、编译入口/类型/构建通过，新调度原生Worker/Tauri/Agent及重启组合留集中验收。 | [SCHED](../apps/web/src/features/plugins/runtime/plugin-scheduler.ts) [SCHEDCONTROL](../apps/web/src/features/plugins/runtime/plugin-schedule-controller.ts) [SCHEDTOOLS](../packages/agent/src/tools/schedule-tools.ts) [API](../packages/plugin-types/src/index.ts) [CTX](../apps/web/src/features/plugins/runtime/plugin-context.ts) [RSS](../plugins/rss-reader/src/index.ts) [ROAM](../apps/web/src/platform/roaming-preferences.ts) [RSSREFRESH](../plugins/rss-reader/src/refresh.ts) [RSSSCHEDULEVIEW](../plugins/rss-reader/src/schedule-view.ts) [RSSSCHEDULEPROOF](../plugins/rss-reader/tests/schedule-view.test.ts) [RSSCACHEPROOF](../plugins/rss-reader/tests/feed-library.test.ts) | Q03 |
| <a id="MORE02"></a>MORE02 | 一次性延迟/短周期/空闲任务与自触发防环 | 部分 | **自动**：维护idle策略；现有Agent查询/批准管理含deferred任务<br>[设计] 宿主自动管线/受控计划 | **部分**：schedules2.0 mode:deferred + defer/cancelDeferred；持久队列/结果与版本绑定<br>[设计] 有owner的具名延迟/空闲任务 | 具名延迟探针；RSS周期任务迁2.0 | 声明式具名回调，不开放任意脚本。1秒到7天延迟，idle为宿主5秒无输入；每秒检查，关闭不运行。最新requestId绑定原参数和dueAt，重复保留回执，另一个排队ID忙，精确ID取消不伤替换任务。排队/开始/结果分别等持久写；开始前落running，崩溃后interrupted不自动重放，未派发的queued重启同版本后继续；插件版本变化取消旧排队意图。回调可安排下一轮，旧完成不覆盖新队列，不并行同任务。64/1024绑定、2/8执行额度、轮转公平；退出排空已派发写，最后绑定释放timer/输入监听。宿主状态禁止普通插件KV伪造；卸载排空后持久清空，重装不复活旧任务。定向测试及隔离Tauri真实Worker排队/退出/重启/执行/清理通过；跨插件事件因果防环与完整业务插件/packaged验收仍缺，不能把限流当防环 | [SCHED](../apps/web/src/features/plugins/runtime/plugin-scheduler.ts) [SCHEDCONTROL](../apps/web/src/features/plugins/runtime/plugin-schedule-controller.ts) [DEFERREDSCHEDULEPROOF](../apps/web/src/features/plugins/runtime/plugin-deferred-schedule.test.ts) [DEFERREDSCHEDULEDESKTOP](../apps/web/tests/desktop/desktop-deferred-schedule-probe.ts) [MAINT](../apps/web/src/features/ai/agent/maintenance.ts) [WORKER](../apps/web/src/features/plugins/runtime/plugin-sandbox.worker.ts) [CTX](../apps/web/src/features/plugins/runtime/plugin-context.ts) | Q04, Q05 |
...
以下“已映射”只保证注册项可追到矩阵行，不意味着目标已实现。Plugin ctx 是授予当前全部 manifest 权限后的 236 个顶层可调用路径；返回的 collection/session 方法单列。Settings 74 路径是在 custom + 主/快模型配置的完整条件快照中生成，不表示未配置 AI 时也显示全部路径。Native command 包含 cfg/no-op 历史项，见 SYS18，不能算桌面能力全部对插件开放。

### Agent global

| 当前注册项 | 矩阵行 | 说明 |
| --- | --- | --- |
| `onboard_reader` | [MEM07](#MEM07) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
...
| `domains.memory.commands.completeOnboarding` | [MEM07](#MEM07) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
...
| `services.schedules.defer` | [MORE02](#MORE02) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
| `services.schedules.cancelDeferred` | [MORE02](#MORE02) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
...
| `storage::onboarding_commit` | [MEM07](#MEM07) | [代码] 内部 IPC 能力证据；不是插件或模型授权入口 |
...
| `storage::identity_work_read` | [MEM08](#MEM08) | [代码] 内部 IPC 能力证据；不是插件或模型授权入口 |
| `storage::identity_work_append` | [MEM08](#MEM08) | [代码] 内部 IPC 能力证据；不是插件或模型授权入口 |
| `storage::identity_work_compact` | [MEM08](#MEM08) | [代码] 内部 IPC 能力证据；不是插件或模型授权入口 |
...
| `storage::plugin_data_snapshot` | [SYS03](#SYS03) | [代码] 内部 IPC 能力证据；不是插件或模型授权入口 |
| `storage::plugin_data_restore` | [SYS03](#SYS03) | [代码] 内部 IPC 能力证据；不是插件或模型授权入口 |
...
| `profile.onboarded` | [MEM07](#MEM07) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
...
| `rss-reader` | [EXT10](#EXT10) | [代码] 源码版本 0.14.0；源码存在不等于打包、安装、启用或模型可调用 |
