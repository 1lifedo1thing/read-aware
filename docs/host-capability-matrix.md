...
| <a id="CON04"></a>CON04 | 跨 Worker RPC 的类型、错误与资源额度 | 部分 | **扩展**：插件 tool 也经过同一 worker bridge<br>[设计] 工具任务不被悬挂 | **部分**：describeContext + callback metadata + Worker 全 envelope 准入/字节/存活句柄预算<br>[设计] 有界可取消版本化 RPC | 所有 Worker 插件及其 Agent 工具 | __fn/__disposable 保持普通数据。Worker→host 的全部 envelope 由同一协议验证字段、类型、正安全整数 ID 和回调元数据；Worker 发出前预检，宿主接收后权威复核，直接 postMessage 不绕过。每条按 UTF-16 字符串/键、完整 backing buffer、Blob/File、容器等核算最多80MiB、100万项/128层，循环/别名不重复核算对象；不是 structured clone 峰值内存或防消息洪泛。单消息10万 callbacks、每激活20万存活 callbacks 在 Worker 和宿主独立计数；宿主按解码图 lease 释放，不信任插件 release。每激活4096 disposable，含在途预留，满额在业务调用前拒绝，不先替换旧贡献。非法消息不派发，有效关联调用报稳定 code，有限元数据释放，日志每激活最多一次；正常错误文本至多4096字符。编码/预检/clone失败释放暂存句柄，退出关闭配额；真实Bun Worker超限/恢复和故障transport基础测试通过。host→Worker镜像/结果的完整准入、全App/时间窗口总量、版本协商、崩溃/撤权/Tauri与packaged跨平台仍缺，GAP07/12/13/17不整体关闭 | [WORKER](../apps/web/src/features/plugins/runtime/plugin-sandbox.worker.ts) [WIRE](../apps/web/src/features/plugins/runtime/plugin-worker-host.ts) [CALLBACKWIRE](../apps/web/src/features/plugins/runtime/plugin-callback-wire.ts) [API](../packages/plugin-types/src/index.ts) [ERRORS](../packages/core/src/errors.ts) [WORKERPROTOCOL](../apps/web/src/features/plugins/runtime/plugin-worker-protocol.ts) [WORKERPROTOCOLPROOF](../apps/web/src/features/plugins/runtime/plugin-worker-protocol.test.ts) [WIREBUDGET](../apps/web/src/features/plugins/runtime/plugin-wire-budget.ts) [CALLBACKBUDGET](../apps/web/src/features/plugins/runtime/plugin-callback-budget.ts) | J12 |
