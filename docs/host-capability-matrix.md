...
| <a id="CON04"></a>CON04 | 跨 Worker RPC 的类型、错误与资源额度 | 部分 | **扩展**：插件 tool 也经过同一 worker bridge<br>[设计] 工具任务不被悬挂 | **部分**：双向全部 envelope 准入、版本握手、字节/存活句柄预算<br>[设计] 有界可取消版本化 RPC | 所有 Worker 插件及其 Agent 工具 | __fn/__disposable 保持普通数据。双向全部 envelope 在发送前预检、接收后复核字段、类型、正安全整数ID；boot共用安装manifest校验，能力/shape/镜像/迁移分别验证。boot/hello/ready显式协议版本，import/activate前握手；版本不符、握手前调用、重复启动或非法镜像失败关闭，不带陈旧状态运行。每条最多80MiB、100万项/128层，完整buffer/Blob/File、字符串/键和容器计数，循环/别名对象不重复；不是克隆峰值或防洪泛。每消息10万callbacks、每激活20万存活callbacks双侧独立计数，宿主按解码图lease释放；4096 disposable含在途预留，在业务调用前拒绝满额。关联非法invoke/result稳定拒绝且下次调用恢复，有限元数据释放；错误文本最多4096字符。所有宿主同步走同一发送口，无法发送则退休该实例；健康/退出发送故障清理等待项并排空。真实Bun Worker超限/恢复/版本故障与宿主transport测试通过。全App/时间窗口总量、完整崩溃撤权、Tauri故障组合与packaged跨平台仍保留，GAP07/12/13/17不整体关闭 | [WORKER](../apps/web/src/features/plugins/runtime/plugin-sandbox.worker.ts) [WIRE](../apps/web/src/features/plugins/runtime/plugin-worker-host.ts) [CALLBACKWIRE](../apps/web/src/features/plugins/runtime/plugin-callback-wire.ts) [API](../packages/plugin-types/src/index.ts) [ERRORS](../packages/core/src/errors.ts) [WORKERPROTOCOL](../apps/web/src/features/plugins/runtime/plugin-worker-protocol.ts) [WORKERPROTOCOLPROOF](../apps/web/src/features/plugins/runtime/plugin-worker-protocol.test.ts) [HOSTPROTOCOL](../apps/web/src/features/plugins/runtime/plugin-host-protocol.ts) [HOSTPROTOCOLPROOF](../apps/web/src/features/plugins/runtime/plugin-host-protocol.test.ts) [WIREBUDGET](../apps/web/src/features/plugins/runtime/plugin-wire-budget.ts) [CALLBACKBUDGET](../apps/web/src/features/plugins/runtime/plugin-callback-budget.ts) | J12 |
