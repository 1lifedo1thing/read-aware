...
that OS policy will permit the next login launch. Real login, packaged paths, external OS controls and cross-platform behavior
need their own evidence. The isolated macOS registration check is recorded in B04.

<a id="file-associations"></a>
## 文件关联与系统文件接收

SET05 has two separate responsibilities: OS handler registration and acceptance
of files delivered by that handler. Registration and file intake have different platform semantics; verification of
