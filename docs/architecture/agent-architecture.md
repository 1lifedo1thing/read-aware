...
工具完整目录与模型默认列表分开。首轮最多 21 个常用阅读、搜索、记忆和设置工具；
其余宿主/插件能力通过 `get_host_capabilities(catalog="tools", query=...)` 发现并加载真实
参数 schema，在下一次请求可调用。每轮最多保留最近加载的 12 个额外工具，新用户轮
重置，失败重试保留；每次请求及执行仍重新检查范围、隐私和宿主可用性。
工具数量和序列化预算由 model-surface 测试约束，新增宿主工具不会自动常驻。

完整目录也按业务对象合并，分组定义见 [tool-families](../../packages/agent/src/tools/tool-families.ts)。
会话查询、阅读统计、正文任务、阅读侧栏、模型目录、临时文件、目录、导入、持久任务、
上下文快照和书中引用分别使用分组入口；涉及写入/控制的分组保留顺序执行。
调用使用有明确分支的 `request.operation`，每个分支只接受自身参数。未启用操作不进入
schema，执行时仍通过原有宿主处理器检查当前范围、权限、版本和取消状态。旧工具名
不作为可调用别名，插件工具仍由插件自己注册。

`get_toc` 默认 `view=chapters` 返回正文检索坐标；`view=navigation` 返回原始嵌套目录及
版本化导航位置，两者不能混用章号。单条与批量标注编辑都走 `apply_annotation_changes`，
保留条件版本写入；删除仍须宿主确认。书内 Agent 直接依据选段/正文解释、翻译、总结；
全局 Agent 通过 `reading_action` 启动当前书的阅读任务，启动回执不等于答案完成。
...
正文抽取和阅读器共用目录章节边界：同文件中的多个目录锚点分开，一章跨越多个
文件时合并其续文。嵌套小节保留为所属章节的位置别名；无链接的分卷标题用于区分
重复章号。`chapterIndex` 只作工具坐标，`get_toc` 不再由它推算章号，`read_chapter`
返回原目录章名。位置反查优先匹配完整锚点，同文件多章的未知位置不猜测为第一章。
正文缓存 v6 会惰性重建旧索引；纪要 v3 使用新坐标，旧纪要不混入宿主图谱或新上下文包，
由既有后台流程逐步补建。历史聊天正文和已存档上下文包不重写。

书籍分类保存两个独立判断：`narrativity` 决定纪要按人物／事件还是概念／论点组织；
`spoilerSensitive` 决定是否保护情节揭晓。小说、历史小说等保留防剧透；真实历史、政治、
传记、回忆录不因叙事写法或尚未读完而限制跨章检索，诗歌、随笔也不因属于文学就默认设限。
正文工具、回答证据检查、图谱与上下文包使用同一策略。读者明确要求只讨论某段或某章时仍遵守其范围。
旧书通过原有分类事件补齐缺失字段，不改已有纪要口径；书内首次问答在需要时先补分类，
使用 fast 模型且最多等待 15 秒，成功后复用持久结果。失败保持原有保守规则，后续节拍可重试。
...
聊天仍保存图片来源 URL、说明；在其他设备上由该设备自行加载和缓存图片。

聊天支持上传/粘贴图片，复用原生资源解码器缩放到最多 2048px，存入同一个本地缓存，
只持久化内容哈希与文件名；缓存淘汰或在其他设备打开时明确显示不可用，不上传图片补齐。
当前附件及上一轮的用户/助手图片可作为视觉上下文。模型声明支持 image 输入时才解码并
传入实际 PNG；`present_web_images` 也给视觉模型返回所选图片像素。每轮书内/网页/用户
图片共用最多 4 张、16 MiB 的输入预算。已展示但像素失败的图片有逐张状态，不能冒充
视觉证据；非视觉模型完全不发送 image block，连视觉模型失败后的重试切换也会过滤。
轮末释放上下文中的像素，后续按本地引用或来源 URL 重新加载，不把 base64 写入转录。
