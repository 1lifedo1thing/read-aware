...
C08补三个真实断点：持久book.graph创建/查看/恢复共用授权补service:llm，避免绕过普通图谱模型权限；插件jobs.get/list/control将调用取消信号传入runner授权等待，取消发生在控制准入前不写意图；恢复读取/授权/检查点失败的任务在当前owner内记录错误并跳过自动重试，释放4并发名额供后续任务，显式resume或新owner可重试。可授权读取的失败queued/running任务投影为needs-attention，原检查点不改写，现有Text Desk任务页沿既有list/get展示并提供恢复。关键权限/不写入、取消、失败队列不饿死/不空转及显式重试检查与受影响类型通过。真实Tauri跨重启/模型执行仍待验，C04/C05余项和C06–C09集中验收继续保留。
