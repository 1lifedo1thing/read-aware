...
本次交接（第75批）：已修复第74批CBR首开2/3：restoreReadingPosition的
相对next改用现有View.init绝对首个linear section。debug新CBR首开1/3、
第三页关书重开保持、EPUB首章通过；4项20断言及web/desktop/Foliate类型通过。
最新隔离包SHA256 d96ae300…运行PID18338，CUA系统导入新CBR首开绿色1/3复验。
debug三本/packaged一本自有源/封面及意图0，原两书/发布包一书四标注保持，
原生选择器已恢复列视图。primary15611/9224保持；勿读取已退出旧CUA包对象。
下一步插件复合过滤、选择/拖动等未覆桌面边界；全格式变体/其他平台/跨设备
仍待验。第72—74已通过故障恢复、图片焦点及四格式入口证据有效，不重跑。
