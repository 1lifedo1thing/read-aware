...
当前 goal（2026-09-13）：用户已启动全量验证，固定实现起点 `b3b2097a` 和当前
243 行矩阵；上一阶段 28 批实现清单见[实现队列](./host-capability-implementation-queue.md)。
现在按[验证覆盖清单](./host-capability-validation.md)组合真实桌面流程，修复阻断既有行为的问题；
开发热更新与桌面操作错开，使用隔离 `com.readaware.app.capability-e2e`，不触碰正式资料。
只重跑受影响步骤；类型/内存端口模型评测/真实 Worker/桌面及外部平台证据分开记录，不推送。

本次交接：第80发布包外部关联确认与退出预览清理已修复复验；第82诊断
导出/日志Reveal通过。第83双隔离Tauri经编译WebDAV Worker双向加密收敛、错误
口令/503恢复及实际连接断开UI通过，自有实例服务已停。第85真实聊天选区/游标
自动ask、拒绝/批准删除、关书重开通过，primary9224原两书/会话hash/配置/15插件
保持。第84修复标注评测假通过：定向模型仍有错误工具调用、笔记标点或选区偏差，
不能报全通过；原首轮统计不变。第81为证据对账，不算新增桌面流程。
用户授权的Astra medium并行已完成第86编译Dictionary retrieve、能力分页与
停用/恢复及旧adapter拒绝，primary9224清理释放；第87双隔离同步积压跨进程恢复、
非零游标/journal二次重启和实际Blob SHA保持通过。第84选区fixture补正后两题
各一次机器通过、人工5/4，均3工具无错误，主代理已复核；仅引文恢复仍有保留。
第88正由sync代理独占9225补编译Reading Goals双书上下文、修改与停用，主代理
维护共享清单。release35546（5afa96a2…）原一书四标注/六builtin保持；桌面再次
锁定，数据目录Reveal等物理界面步骤等待解锁，其余验证继续。goal工具最新状态
为paused；本轮依用户“继续”指令执行，未尝试改写其调度状态或标记完成。
