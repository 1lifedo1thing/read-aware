...
真实桌面已新增二十一组组合证据：正文准备/历史、图片脚注、RSS缓存及重排、
版本化标注、记忆分页/纠错/遗忘、设置事务/观察、分句及系统语音回退、
记忆巩固冲突及图谱围栏、原生文件/目录/导出/重复导入、备份UI恢复/重载屏障、
Jumper导航与历史、实时视图订阅/草稿/退役、时长结算及正文状态、正文任务共享/取消/退役、Text Desk实际远端识图、画像/合并身份/上下文归档导出与失效、会话隔离/环境快照/词典缓存跟随、网络重试/延迟执行/流量隔离、回调生命周期/文档保真/合成凭据加密事件、插件数据事务/实际升级回滚/降级/卸载、七种格式导入/正文及加密MOBI拒绝读取。具体范围见[验收记录](./evidence/2026-09-13-full-validation/README.md)。
用户已解锁，前台选区和朗读模式截图有效；原生选择器可操作。首次后台选区
消失在新书后台→前台复验中未重现，原观察仍保留；中途恢复了FB2与PDF截图，随后PDF复验时WebView又hidden，已请求用户保持测试窗口可见。PDF可见正文另一次重开恢复available，时序差异待核。

已修复 settings 越权稳定码和文件选择器拒绝 fb2.zip 的导入阻断。
PDF无效href泄漏JSON异常已修，14项检查及实际engine adapter拒绝不移动通过；完整Worker导航受hidden绘制超时阻塞。
...
Tauri Text Desk识图通过但Agent错ID/复验超时失败；画像/身份/context原生与Worker局部通过，访谈UI/模型注入/其他recipe仍待验；生命周期/权限/后台任务、
