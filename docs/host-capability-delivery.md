...
当前目标：[九类缺失能力补全](./host-capability-completion-goal.md)，基线 `4657c3d0`。C01按书记忆/图谱/归档、C02本书会话、C03受限工作区/命令已接，真实桌面组合待集中验收。C04本批补开书ready/fail及book.opened、实际控件渲染、显式选区创建/清空和播放异步反馈的来源；旧音频回调不能确认或推进通知中接管的新播放，迟到控件回执不能重标用户的新状态。70项定向及25项相邻检查、web/desktop类型与Foliate源校验通过，受控适配器不作Tauri证据。下一步接Foliate导航/rendered的明确请求来源、模式配置/分段/位置写及播放输入变化，再开放observeSession的reaction参数；另余标注/记忆等快照、设置目录/凭据失效及其他派生路径。C04仍未完成，C05–C09仍未实施且全部保留。此前plugins1.3设置/私有文档/投影观察及消费者的已过证据保留在目标文档，不重复归零。下列旧active goal均为历史任务。
