...
当前目标：[九类缺失能力补全](./host-capability-completion-goal.md)，基线 `4657c3d0`。C01按书记忆/图谱/归档、C02本书会话、C03受限工作区/命令已接，真实桌面组合待集中验收。记忆/图谱观察来源批次已提交 `b6ced5ff`。C04本批把全书/受限插件与Agent面板开关/宽度的actor传到控件、KV补丁、瞬时状态及实际React提交，渲染只合并本次变化输入；KV乐观/成功/回滚保留各请求来源，更新覆盖和同键重入不能被旧通知重标。reading-ai-actions章节准备/openChat已沿同一来源，Agent显式绑定agent。37项定向检查（含18个隔离React场景）、21项相邻检查和web/desktop类型通过，日志见目标文档；受控IPC/React/脚本Agent不作Tauri/SQLite/真实模型证据。下一步接面板null/解绑投递、自动chat焦点、窗口响应式布局、原生选区/图片及阅读设置重排的来源，再接贡献注册/退休、设置目录/凭据和其他观察来源。面板observe及reading.observeSession的公共reaction仍未开放，对应自动消费者仍待迁移。C04未完成，C05–C09未实施且全部保留，下列旧active goal为历史任务。
