...
真实桌面已新增十七组组合证据：正文准备/历史、图片脚注、RSS缓存及重排、
版本化标注、记忆分页/纠错/遗忘、设置事务/观察、分句及系统语音回退、
记忆巩固冲突及图谱围栏、原生文件/目录/导出/重复导入、备份UI恢复/重载屏障、
Jumper导航与历史、实时视图订阅/草稿/退役、时长结算及正文状态、正文任务共享/取消/退役、Text Desk实际远端识图、画像/合并身份/上下文归档导出与失效、会话隔离/环境快照/词典缓存跟随。具体范围见[验收记录](./evidence/2026-09-13-full-validation/README.md)。
用户已解锁，前台选区和朗读模式截图有效；原生选择器可操作。首次后台选区
消失在新书后台→前台复验中未重现，原观察仍保留；中途再次锁屏后当前Tauri截图已恢复，FB2与PDF画面可见。PDF可见正文快照仍not-visible，单列待核。

已修复 settings 越权稳定码和文件选择器拒绝 fb2.zip 的导入阻断。测试探针的
摘要源版本、Jumper实时视图订阅契约已更新。当前桌面自有书/集合均清理，原
