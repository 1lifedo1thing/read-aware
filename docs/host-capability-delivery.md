...
1. 导出已接完整加密归档；原生执行器已将数据库、blob/程序文件、插件数据和凭据组成共同恢复，真实加密归档到隔离数据库/文件的定向验证已通过。宿主已实现私有Worker探测/迁移调用链，桌面入口尚未接；真实插件与产品链路仍需验收，完成条件1尚未全部满足。
2. 桌面导出入口已接，完整导入的口令、预检/确认、执行与结果界面仍未接，条件2尚未满足。
3. 完整原生执行器已接文件journal与SQLite共同决定，安装文件后的取消会共同回滚；既有进程退出测试通过。宿主运行时接线及真实Tauri中的失败结果还需验收，条件3尚未全部满足。
4. 真实 Tauri 备份与恢复验收尚未进行；恢复主流程接通后在隔离测试资料库执行代表性成功与失败场景，条件4尚未满足。
5. 保留全部任务成果，已验证单元自动提交且不推送；最终账本须记录实际桌面结果，全部条件满足后结束此 goal，不续建其他能力 goal。

必要前置仅限具体失败证据：当前候选检查已证明混选父子记录会破坏FK/聊天归属，别名混选会形成跳转链，因此保留本轮检查。其他尚未阻碍验收的改进不继续扩展。
