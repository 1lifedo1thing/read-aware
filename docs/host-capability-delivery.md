...
当前全量 goal（2026-09-12）：固定 `91c2c4e9` 的矩阵/责任模型，连续补完全部既定
实现与接线缺口；[剩余交付队列](./host-capability-implementation-queue.md)保存分批状态。
本阶段不运行逐行 Tauri/Worker/全格式/跨平台验收，不以 pending 重新施工。
每批完成实际宿主 → Agent/Worker → 适用第一方消费者，通过定向检查后自动提交，不推送。
一批完成后继续下一批，只有全量范围满足才结束 goal。

当前状态：固定清单实现/接线已完成至第二十八批。MEM04原子新事实准入/遗忘保护/读与模型预算、LIB14同身份同文虚拟位置/标注重排迁移已接；UI03现有18项命令核实无需再造入口。按下方各行语义记录限制，所有新增真实Tauri/Worker/模型/全格式/平台组合仍集中pending，未把矩阵“部分”机械改为全部已验。
已完成并复用：试跑 `91c2c4e9` 的版本化 Range 创建标注；本轮 RSS 缓存回收、TXT05 暂停/恢复及 OPS03/OPS11 宿主确认修复
实现/接线与定向检查完成（下节），不把各行剩余边界或集中验收归零。
