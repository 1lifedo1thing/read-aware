...
当前目标：[九类缺失能力补全](./host-capability-completion-goal.md)，基线 `4657c3d0`。接线阶段：C01 memory2.8按书记忆/图谱/归档（`1b1aafe9`）及C02 conversations1.5本书会话/状态/待批准回合/停止清空已接，Memory Desk0.15消费授权书入口。定向授权/取消/资源/事件/清空派发、编译插件路径、工作区类型、capability及生成检查通过；本次真实Tauri/Worker/Agent消费者组合验收仍待集中执行。下一步C03受限ui.commands/ui.workspace投影与导航；C03–C09全部保持范围且未实施。具体条件与证据边界见目标文档；下列原active goal均为历史任务。
