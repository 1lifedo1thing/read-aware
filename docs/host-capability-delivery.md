...
当前目标：[九类缺失能力补全](./host-capability-completion-goal.md)，基线 `4657c3d0`。C01按书记忆/图谱/归档、C02本书会话、C03受限工作区/命令已接，真实桌面组合待集中验收。C04阅读器模式批次已提交 `e5866c7c`。本批plugins1.4接 annotations.events.observe 第二参数来源：等待短持久写回执，跨提交样本重读，失败回调/持续读错误/恢复保留因果；已知书/ID过滤，ID-only事件保守失效授权页，未变化的成功查询退休旧原因。受限包装等待实际回调，Annotation Desk0.8绑定自动视图发布。公开A→B→A在all/book授权下拒绝循环，独立用户根可继续；实际Bun Worker跨await条件修改、观察器/消费者/类型通过（日志见目标文档），受控IPC不作Tauri/SQLite证据。下一步接记忆查询与图谱任务快照的真实来源，再开放对应reaction；还余阅读器设置重排、原生选区/焦点、面板/图片、贡献注册/退休等派生来源及observeSession、设置目录/凭据失效和其余第一方自动消费者。C04仍未完成，C05–C09仍未实施且全部保留。既有已过证据不重复归零，下列旧active goal为历史任务。
