...
本次交接（第66批）：Text Desk0.22真实Worker从CBR图片列表/预览打开灯箱，
缩放/旋转/平移/复位、Show image退出全部插件界面保留灯箱、关闭通过；宿主
旧viewer ID拒绝误控新图。补回READ12遗漏的2026-09-11 FB2历史证据，仅记部分。
本批无代码修改/新门禁；第65批代码7项/26断言、web/desktop/Foliate类型仍有效。
自有CBR/源/封面清零，原两书及会话hash保持，primary PID15611/9224。
CUA最近仍Mac locked、已反馈，不重复轮询。release PID86059仍为a00eb3fa，
不含第65批修复，相关验收前须重建。下一步未覆格式/大图适配；桌面可控后接续
原生选择器、画面/焦点和发布包。物理手势、RAR变体、其他平台/跨设备仍待验。
