...
当前目标：[九类缺失能力补全](./host-capability-completion-goal.md)，基线 `4657c3d0`。C01按书记忆/图谱/归档、C02本书会话、C03受限工作区/命令已接，真实桌面组合待集中验收。C04本批接通宿主导航→Foliate异步装载/定位/渲染→选区清空/忙碌/外壳反馈的请求来源；固定版式同页重叠装载及迟到目标只允许最新导航显示，同跨页左右切换报告正确页面。51项宿主、1项编译View/FixedLayout协议及6项相邻契约检查、53模块构建和web/desktop类型通过；DOM受控装载不作Tauri证据，新增原生场景待集中运行。下一步接模式配置/分段/位置写及播放输入来源，设置重排与原生选区/焦点等派生反馈，随后开放observeSession的reaction参数；另余标注/记忆等快照、设置目录/凭据失效和第一方自动反应消费者。C04仍未完成，C05–C09仍未实施且全部保留。既有已过证据保留在目标文档，不重复归零。下列旧active goal均为历史任务。
