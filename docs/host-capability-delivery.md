...
当前目标：[九类缺失能力补全](./host-capability-completion-goal.md)，基线 `4657c3d0`。C01按书记忆/图谱/归档、C02本书会话、C03受限工作区/命令已接，真实桌面组合待集中验收。导航批次 `a183b681` 已提交；C04本批接模式配置→异步分段/落点→位置保存/播放输入的来源，旧回执不能重标新用户状态。外部模式偏好从成功KV提交的具体值与来源驱动，配置自身提交不再次触发，失败乐观写不制造新意图；等待写入期间只采用最新偏好。宿主/React hooks/受控IPC组合及相邻播放、写入、步进检查和web/desktop类型通过，不作Tauri/真实Worker/音频证据。下一步接设置重排、原生选区/焦点、面板/图片及贡献注册/退休等派生来源，再开放observeSession的reaction参数；标注/记忆观察器目前只做轮询，需接实际变化来源后开放reaction。另余设置目录/凭据失效和第一方自动反应消费者。C04仍未完成，C05–C09仍未实施且全部保留。既有已过证据保留在目标文档，不重复归零。下列旧active goal均为历史任务。
