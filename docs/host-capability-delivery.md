...
中断预跑）。修复及重评分单列，不能与首轮混算；主 Agent 质评仍在继续。

真实桌面已新增十二组组合证据：正文准备/历史、图片脚注、RSS缓存及重排、
版本化标注、记忆分页/纠错/遗忘、设置事务/观察、分句及系统语音回退、
记忆巩固冲突及图谱围栏、原生文件/目录/导出/重复导入、备份UI恢复/重载屏障、
Jumper导航与历史、实时视图订阅/草稿/退役。具体范围见[验收记录](./evidence/2026-09-13-full-validation/README.md)。
...
剩余：全量失败及通过抽查/题库外复核，标注定向修复已保持附件边界，但仍有冗长/漏答/超时；
