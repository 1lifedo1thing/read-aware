...
真实桌面已新增十九组组合证据：正文准备/历史、图片脚注、RSS缓存及重排、
版本化标注、记忆分页/纠错/遗忘、设置事务/观察、分句及系统语音回退、
记忆巩固冲突及图谱围栏、原生文件/目录/导出/重复导入、备份UI恢复/重载屏障、
Jumper导航与历史、实时视图订阅/草稿/退役、时长结算及正文状态、正文任务共享/取消/退役、Text Desk实际远端识图、画像/合并身份/上下文归档导出与失效、会话隔离/环境快照/词典缓存跟随、网络重试/延迟执行/流量隔离、回调生命周期/文档保真/合成凭据加密事件。具体范围见[验收记录](./evidence/2026-09-13-full-validation/README.md)。
