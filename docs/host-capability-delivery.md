...
真实桌面已新增三十二组组合证据：正文准备/历史、图片脚注、RSS缓存及重排、
...
消失在新书后台→前台复验中未重现，原观察仍保留；中途恢复了FB2与PDF截图，随后PDF复验时WebView又hidden；用户再次确认解锁后unminimize恢复visible，完整Worker导航及面板DOM焦点已通过。PDF首次正文快照仍有短暂空值，时序差异待核。第23流程中窗口再次hidden，导航/设置和DOM通过，连续绘制帧采集待验。第25流程真实Worker与Agent阅读设置重置/继承、设置UI九项修改及原生重读通过并恢复；FB2实际正文21px/justify↔17px/start及停用覆盖恢复通过；其余排版/固定版式颜色与进程重启待验。阅读外观UI无独立重置按钮，CFG03沿既定共享命令验收。PDF本次后台未就绪拒绝外观操作，未记通过。第26流程编译Workspace Profiles v2十项保存/应用、真正分页/旧游标、拒绝删除/批准保存与删除/过期工作区及已删除目标保护通过；已恢复设置并清空自有预设/Worker。第27流程插件快捷键冲突/重绑/实际WebView按键/默认恢复、原生最大化/最小化/恢复及Worker实时显示通过；全屏请求后仍false，保留待验。原KV/窗口状态和Worker均清理恢复。第28流程字体实际翻页/搜索、Lora与Menlo应用、跟随阅读，以及Foliate正文/聊天输入框字体与Lora加载通过；恢复Inter并清理Worker。第29流程八项菜单visible/overflow、非法整批拒绝、书架/阅读器/选区实际DOM与溢出入口通过；Agent关闭/恢复五个AI开关对应入口变化通过，已恢复菜单/开关并退役Worker。第30流程预设跨Worker重启和v1/v2实际Apply通过，旧回调退役拒绝、本书覆盖保留；新增restart探针类型通过，已清理测试文档/Worker并恢复设置。第31流程修复真实关闭权限遗漏；debug正常关闭/重启26项KV与合成业务资料保留通过；release包原生关闭按钮、原生导入FB2/实际正文/翻章/原生退出后同书同位置重读通过。发布包保留一份合成书，详见证据。第32流程release物理拖选高亮/笔记、列表定位、独立内容Lora/XL/relaxed实际呈现及进程重启保留通过；排版恢复默认，保留合成书和两条标注。接续其他设置组合、聊天消息字体、菜单编辑器/原生快捷键与插件全屏等未覆边界，沿用现有探针。
