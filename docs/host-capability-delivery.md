...
当前 goal（用户于 2026-09-12 明确收敛）：完成 OPS08 完整备份与恢复，让用户在真实 Tauri 桌面应用中完成备份、预检、必要确认和恢复，并核对正确结果。按既定数据覆盖及恢复语义推进，优先恢复执行、原子失败恢复和桌面入口；其他能力只保留为项目待办，不纳入本 goal。下方旧记录保留其当时范围。本文件是执行账本，不替代[统一模型](./host-capability-model.md)或[当前矩阵](./host-capability-matrix.md)。
...
本 goal 的五项完成条件均已满足；本次只关闭 OPS08，不继续其他能力。完整复现步骤、夹具与证据边界见 [桌面验收记录](./acceptance/ops08-backup-2026-09-12.md)。
