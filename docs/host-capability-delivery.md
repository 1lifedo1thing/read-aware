...
消失在新书后台→前台复验中未重现，原观察仍保留；中途恢复了FB2与PDF截图，随后PDF复验时WebView又hidden；用户再次确认解锁后unminimize恢复visible，完整Worker导航及面板DOM焦点已通过。PDF首次正文快照仍有短暂空值，时序差异待核。第23流程中窗口再次hidden，导航/设置和DOM通过，连续绘制帧采集待验。第25流程真实Worker与Agent阅读设置重置/继承、设置UI九项修改及原生重读通过并恢复；实际阅读器排版/固定版式颜色、UI重置入口与进程重启仍待验。
