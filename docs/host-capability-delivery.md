...
session2.16新增settings.refreshModelCatalog(provider)可用性查询：严格参数、模型发现与网络权限先检，未授权不读目录状态；共享实际公开提供者判定，刷新中可加入，远端健康保持unknown，不要求账号密钥。实际刷新保留权限/提供者重验与共享任务取消语义。Agent双scope与Maintenance Desk0.14目录条件页已接，条件页不刷新，明确动作才请求。Web/desktop、Agent及插件类型/构建、权限/无网络查询/取消/错误回执和插件消费者检查通过；真实Tauri与公共目录网络调用未验。当前继续C04其余来源和C05其余操作条件，C06–C09保留已有接线，集中桌面/重启/模型验收待执行；不计完整目标完成。
...
当前目标：[九类缺失能力补全](./host-capability-completion-goal.md)。C01–C03已接、待集中桌面验收；C04仍缺原生异步/响应式与其他来源，C05推理/朗读/模式/正文准备已接、其余条件仍缺。本批C06 plugins1.8已接版本化服务目录、双边权限/书域、独立Worker及取消/换代清理；补书服务私有文档宿主过滤/CAS和全局KV镜像隔离，Jumper0.10→Text Desk0.27消费本书书签分页。实际Bun双Worker和受控存储、运行时与编译消费者已通过；Tauri/SQLite组合尚未执行。C06通用Agent服务入口已接逐次冻结批准、书域和阅读政策约束，实际Agent工具→Bun Worker通过；真实批准UI/模型仍待集中验收。C07已接原生同事务/持久回执、宿主语义预览/条件撤销、transactions1.0公共Worker、插件书域/权限/观察/排空与Agent冻结批准，Workspace Profiles0.8联合预设版本检查和设置应用并可撤销。原生SQLite、受控Agent批准、实际Bun Worker拒权及编译消费者/类型与库存通过，真实Tauri三域组合、重启回执/撤销待集中验收。C08已写schema47持久检查点与恢复调度底座，控制意图先持久化，未知结果不盲重放；SQLite重开/修订围栏和取消核对关键检查及受影响类型通过。C08事务步骤执行器已接冻结事件身份/提交内容、当前授权复核和持久回执核对；正文/图谱步骤已接统一宿主执行器、终态检查点和物理排空；显式恢复意图已持久化；图谱重建持久章节计划/生成结果/事件回执核对已接，插件激活恢复和jobs1.0根上下文接口已接逐步骤授权/书域/私有文档及退休排空，Text Desk0.28可逐本后台准备当前页并查看/控制任务。Agent启动/恢复逐次批准、四个任务工具、书域与私有文档限制、启动恢复/退出排空已接。图谱恢复复用冻结生成结果、原条件写及当前阅读边界，计划/结果先落盘；反应因果续接仍有缺口；真实批准UI/Worker/Tauri跨重启验收仍未验证。C09已写schema48事务内元数据日志和持久owner/selector/epoch游标，schema49补按书事件归属、多书/集合展开及按书设置通知；公共查询契约和宿主安全通知投影已写。changes1.0插件权限/书域/设置目录绑定及Agent两个续读工具已接，受影响类型通过；Text Desk0.29已接持久页面与游标、失败不前移、过期重建基线；消费者类型/构建及关键持久化检查通过，恢复边界已补wipe后epoch重建、真实自增高水位、bootstrap及显式投影重建失效，SQLite定向检查通过；真实Worker/Tauri及完整恢复运行验收仍待补。C05已补session2.4 sync.now连接/凭据/提供者只读条件，执行重验，Maintenance Desk0.6消费者已接；受影响类型/构建及关键撤权检查通过。C04窄屏响应式面板已接匹配窗口来源、晚到结果失效和因果发布，React关键检查通过；Foliate分页/固定版式内部窗口重排也已接宿主桥及过期结果淘汰，严格类型与调度边界检查通过；原生失败/动画后续帧/元素自身及任务因果等余项仍保留。plugins1.9已补可选稳定事件ruleId、宿主有界因果序列化和RSS0.22清理消费者，关键跨激活身份检查通过；jobs1.1已接创建时不可变source、按任务执行器恢复和具名反应启动；SQLite重开/不可替换来源及恢复防重复、取消边界通过，真实Worker/Tauri链待验。schema50已把AI秘密密文行的新增/轮换/删除映射为不含秘密槽和值的设置失效，credentialConfigured接入已授权游标；秘密通知和插件写入保留因果来源。SQLite回滚/不泄密、秘密队列失败/防环及Web/desktop类型检查通过。session2.5已补图谱生成只读条件与派发重验，Memory Desk0.17批准前条件页和Agent入口已接；受影响类型/构建、权限书域/容量和消费者检查通过。reading2.22已接observeSession反应凭证/稳定规则及受限回调await，Jumper0.11按钮状态、Text Desk0.30与Listening Desk0.12视图消费者已迁移；权限内异步反应/防重复/过期及用户新根、受影响类型/构建和消费者流程通过。macOS新增原生输入序号，HostWindowService按真实输入世代保留命令后续几何来源，读失败/不匹配时先复核世代再延续；新输入使旧来源失效，包括尺寸相同的采样。Web/desktop类型、cargo离线编译和窗口/响应式关键检查通过；真实AppKit/Tauri、其他平台及外部窗口管理器来源仍待补，完整C04未闭合；设置目录已改为从贡献数组及有来源的当前模式选择读取因果，命令/字体/主题/快捷键目录反馈不再重建系统根；定向来源防环及Web/desktop类型通过。reading2.23已接临时强调写/清/渲染反馈与observeEmphasis反应凭证，Foliate覆盖层创建及固定页预加载/PDF重绘透传来源；受限插件现可经当前书/owner/revision校验清除自有标记，Text Desk0.31已迁移。类型/构建、关键控制器和公共书域防环检查通过，真实Tauri待验。凭据漫游自动来源、安装状态/Worker错误及C04/C05余项继续补齐，再集中真实验收。未完成范围保持，不推送。
