...
1. 导出已接完整加密归档；现有逐行选择已能在隔离SQLite中实际恢复并经重放保留，文件/插件/凭据尚未与其组成完整恢复，完成条件1尚未满足。下一步把数据库事务接入文件安装和失败恢复，再接桌面入口。
2. 桌面导出入口已接，完整导入的口令、预检/确认、执行与结果界面仍未接，条件2尚未满足。
3. 已验证数据库恢复结果、重放保留与事务失败回滚；文件journal与SQLite提交记录已验证进程退出后的共同恢复，但完整导入执行器尚未接入该机制，条件3尚未满足。
