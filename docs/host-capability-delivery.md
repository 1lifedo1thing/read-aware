...
已定位并消除此前两项图谱退出检查超时：旧Worker桥fixture未满足新增图谱准入，缺书时执行器根本未派发，检查却等待entered。此退出检查现在明确替换准入边界，只验证已经派发的图谱执行在stop/crash时收到取消、物理执行结束前宿主不退休、结束后Worker释放。两项定向检查和受影响类型通过，不作为真实Worker/Tauri或图谱准入成功证据。模型目录缓存不会触发公开设置观察，本次未发现该路径的反馈循环，未添加无消费者的来源扩展。C04原生窗口派生来源与C05其余操作条件仍待接通，C06–C09真实桌面/重启/模型验收保留。
