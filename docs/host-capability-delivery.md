...
本次交接（第71批）：复用批准探针挂载真实ChatInteractionPrompt并连接生产
Agent shelf工具/原生端口，补齐上轮缺少onUpdate的呈现。集合拒绝/取消保留，
批准后集合null、成员归属null而书/源保留；单书全局拒绝、书内默认ID取消保留，
书内批准后书目/原文件删除。不是完整聊天轮次/LLM/物理点击证据。
仅扩展现有探针，desktop类型通过，无产品改动；自有书/源/封面/集合及批准UI0，
原两书/空集合及会话hash保持。primary PID15611/9224、旧release86059保持。
第68批Format Validation.app不含69/70，实际packaged验收前集中重建；CUA最近
locked/WebView hidden，不重复轮询。下一步未覆消费者与故障边界；桌面可控后
接续新包、原生选择、旋转终态/焦点。全格式变体/其他平台/跨设备仍待验。
