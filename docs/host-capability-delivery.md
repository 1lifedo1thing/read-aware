...
当前目标：[九类缺失能力补全](./host-capability-completion-goal.md)，基线 `4657c3d0`。接线阶段：C01 memory2.8按书记忆/图谱/归档（`1b1aafe9`）、C02 conversations1.5本书会话（`985a5a61`）、C03 ui1.15受限工作区投影/导航与宿主命令已接；Memory Desk0.15、Library Desk0.13消费。C03选择集分页前授权过滤、私有版本、范围校验和切书取消的定向检查通过；新路径真实Tauri/Worker/Agent消费者组合验收仍待集中执行。下一步C04跨插件事件因果防环；C04–C09全部保持范围。具体条件与证据边界见目标文档；下列原active goal均为历史任务。
