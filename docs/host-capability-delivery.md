...
当前目标：[九类缺失能力补全](./host-capability-completion-goal.md)，基线 `4657c3d0`。C01按书记忆/图谱/归档、C02本书会话、C03受限工作区/命令已接，真实桌面组合待集中验收。图片/面板观察来源已提交 `7494f0fb`。C04本批接settings→KV镜像/回滚→全局/本书/auto配色有效设置→字体/CSS/宽度间距/固定页配色的来源，其他书或不生效的变化不重绘。字体await前捕获context，较早请求、替换阅读器与卸载丢迟到结果；移除漫游汇总对已通过KV通知的重复重标。Foliate属性批次/字体展开/固定配色与布局反馈沿用context，实际公开设置、React/受控IPC/回滚与编译引擎协议通过；真实字体/布局Tauri场景已加未跑。下一步接布局模式切换重建整引擎的来源、窗口响应式布局/图片ResizeObserver、自动chat/灯箱与显式焦点/原生选区，再接贡献换代、设置目录/凭据及其余观察来源。reading.observeSession的公共reaction仍未开放。C04未完成，C05–C09未实施且全部保留；新增Tauri/SQLite/模型验收仍集中后置。
