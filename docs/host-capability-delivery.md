...
真实桌面已新增二十九组组合证据：正文准备/历史、图片脚注、RSS缓存及重排、
版本化标注、记忆分页/纠错/遗忘、设置事务/观察、分句及系统语音回退、
记忆巩固冲突及图谱围栏、原生文件/目录/导出/重复导入、备份UI恢复/重载屏障、
Jumper导航与历史、实时视图订阅/草稿/退役、时长结算及正文状态、正文任务共享/取消/退役、Text Desk实际远端识图、画像/合并身份/上下文归档导出与失效、会话隔离/环境快照/词典缓存跟随、网络重试/延迟执行/流量隔离、回调生命周期/文档保真/合成凭据加密事件、插件数据事务/实际升级回滚/降级/卸载、七种格式导入/正文及加密MOBI拒绝读取、解锁后PDF导航/面板焦点、应用导航/集合多选/宿主命令权限与持久设置、书目/收藏/集合管理与批准删除/恢复后清理保护、阅读设置三目标覆盖/重置/继承与实际设置界面、Workspace Profiles预设保存/应用/确认删除及Agent宿主批准/过期保护、快捷键冲突/重绑/默认恢复和窗口状态观察、字体目录翻页/搜索/应用及实际正文与输入框字体加载、八项菜单配置/实际书架阅读选区入口与五项AI入口开关。具体范围见[验收记录](./evidence/2026-09-13-full-validation/README.md)。
用户已解锁，前台选区和朗读模式截图有效；原生选择器可操作。首次后台选区
消失在新书后台→前台复验中未重现，原观察仍保留；中途恢复了FB2与PDF截图，随后PDF复验时WebView又hidden；用户再次确认解锁后unminimize恢复visible，完整Worker导航及面板DOM焦点已通过。PDF首次正文快照仍有短暂空值，时序差异待核。第23流程中窗口再次hidden，导航/设置和DOM通过，连续绘制帧采集待验。第25流程真实Worker与Agent阅读设置重置/继承、设置UI九项修改及原生重读通过并恢复；FB2实际正文21px/justify↔17px/start及停用覆盖恢复通过；其余排版/固定版式颜色与进程重启待验。阅读外观UI无独立重置按钮，CFG03沿既定共享命令验收。PDF本次后台未就绪拒绝外观操作，未记通过。第26流程编译Workspace Profiles v2十项保存/应用、真正分页/旧游标、拒绝删除/批准保存与删除/过期工作区及已删除目标保护通过；已恢复设置并清空自有预设/Worker。第27流程插件快捷键冲突/重绑/实际WebView按键/默认恢复、原生最大化/最小化/恢复及Worker实时显示通过；全屏请求后仍false，保留待验。原KV/窗口状态和Worker均清理恢复。第28流程字体实际翻页/搜索、Lora与Menlo应用、跟随阅读，以及Foliate正文/聊天输入框字体与Lora加载通过；恢复Inter并清理Worker。第29流程八项菜单visible/overflow、非法整批拒绝、书架/阅读器/选区实际DOM与溢出入口通过；Agent关闭/恢复五个AI开关对应入口变化通过，已恢复菜单/开关并退役Worker。接续预设跨重启、聊天消息/笔记实际字体、菜单编辑器/其他快捷键与关闭/全屏等未覆边界，沿用现有探针。
