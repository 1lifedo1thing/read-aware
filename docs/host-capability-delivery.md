...
当前进度：分阶段基础门禁修复后通过，Web 构建通过；本机原生 465 pass/2 ignored。
Agent 6 个工具遗漏、可读到期时间、Host 导入 mock、协商版本及图片用例类型均已修正。
真实 Text Desk 已验 FB2 导入→正文准备→完成历史→Worker 重启后重读、实际图片与宿主灯箱
缩放旋转、脚注预览与宿主浮层；成功轮已关书/删除/释放文件/恢复插件，见[首批证据](./evidence/2026-09-13-full-validation/README.md)。
英文剧透授权漏认已修复，2 个定向模型样本及题库外问答完成；机器通过的固定题仍因冗长/
推测过强在 Viewer 记为有保留。全量模型首轮仍运行，所有失败/相关样本和真书样本待继续质评。
RSS 已完成订阅→503 保留缓存→Worker 重启离线读→源更新重开→文章重排恢复位置→退订清理。
标注已验真实插件创建下划线/引用笔记、并发冲突拒绝、刷新编辑、改色绘制和批量删除。
首次后台新书打开后选区消失仍待复现定位；前台重开成功不关闭该观察。
Judge 缺交互回执误判及过期表单/记忆契约已修；跳过选择后猜目标的行为已修并定向复核。
记忆已验候选落盘/去重、106 条真实分页、纠错置顶和同文遗忘抑制；隐藏桌面下仅计
Worker/控件状态/SQLite，视觉待会话恢复。纠错后遗忘不抑制不同文本的原目标再提交。
Library Desk 私有封面保存/重启读/删除及新隔离库加密备份解密预检已通过；
旧验收库四条遗留缺源正确阻止导出，未改旧数据。当前仍锁屏，所有新画面证据待验。
章节编号和记忆查询范围已定向修正/复核；冗余调用、漏答和模型超时仍待处理，
60秒独立诊断捕获模型连续推理且未调用工具。全量真书第3组仍运行，勿重跑已过基线。
设置事务/授权过滤/观察和分句/系统朗读回退/面板控制已新增原生证据；
settings 越权稳定码已修并真实 Worker 复验。所有这两批自有资源已清理。
当前继续记忆图谱/巩固并发、画像/身份/context、权限生命周期、实际恢复与
原生资源；未验平台/格式/跨设备保留明确边界，不得以首批通过结束全量 goal。
