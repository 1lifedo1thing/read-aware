...
真实桌面已新增十六组组合证据：正文准备/历史、图片脚注、RSS缓存及重排、
版本化标注、记忆分页/纠错/遗忘、设置事务/观察、分句及系统语音回退、
记忆巩固冲突及图谱围栏、原生文件/目录/导出/重复导入、备份UI恢复/重载屏障、
Jumper导航与历史、实时视图订阅/草稿/退役、时长结算及正文状态、正文任务共享/取消/退役、Text Desk实际远端识图、画像/合并身份/上下文归档导出与失效。具体范围见[验收记录](./evidence/2026-09-13-full-validation/README.md)。
用户已解锁，前台选区和朗读模式截图有效；原生选择器可操作。首次后台选区
消失在新书后台→前台复验中未重现，原观察仍保留；随后桌面控制工具再次确认锁屏，已异步通知用户；当前前台视觉待解锁，原生数据链路继续。
