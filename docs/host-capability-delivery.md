...
当前目标：[九类缺失能力补全](./host-capability-completion-goal.md)，基线 `4657c3d0`。C01按书记忆/图谱/归档、C02本书会话、C03受限工作区/命令已接，真实桌面组合待集中验收。显式面板调用来源已提交 `1f6aaf2b`。C04本批plugins1.6接图片/面板observe的独立delivery，关闭null与解绑、串行合并和回调失败后续通知保留来源；图片读取/打开/实际变换/关闭沿Agent或插件actor，原生接管不能确认旧控制，迟到清理/旧按钮不能关闭新图。Listening Desk0.11/Text Desk0.23绑定自动发布而保留独立用户动作。公开all/book图片及面板双插件循环拒绝和独立根、真实Bun Worker空快照跨await、受控IPC/React/消费者与类型通过；面板自动后续操作可取代尚未确认的旧请求，旧回执仍为superseded，最终状态及持久值另验。下一步接窗口响应式布局/图片ResizeObserver、自动chat/灯箱焦点、原生选区及阅读设置重排，再接贡献注册/退休、设置目录/凭据和其余观察来源。reading.observeSession的公共reaction仍未开放。C04未完成，C05–C09未实施且全部保留；新增真实Tauri/SQLite/模型验收仍集中后置。
