...
当前目标：[九类缺失能力补全](./host-capability-completion-goal.md)，基线 `4657c3d0`。C01按书记忆/图谱/归档、C02本书会话、C03受限工作区/宿主命令已接，真实桌面组合待集中验收。C04仍实施中：plugins1.3 在既有事件/Worker因果链上接设置快照、书库/会话投影失效及私有文档观察，第二回调参数供withEvent使用；合并保留按根路径，独立新根可继续，重复根拒绝。私有文档等待匹配写回执、跨提交重读、冲突/失败不发、回调重试和读取错误恢复保留来源。Jumper0.9/Workspace Profiles0.7已消费，事件/快照公共防环、受控文档事务及真实Bun Worker第二参数检查通过，插件构建/类型和82项受影响回归通过；check:capabilities所含检查全部通过（修正两处旧测试夹具后，从运行时继续，复用前半已过检查）；另14项书籍/摘要副作用回归通过。下一步接阅读器异步反馈、其他快照、设置目录/凭据失效及剩余派生来源；C04不能计完成，C05–C09仍未实施且全部保留。具体证据见目标文档。下列旧active goal均为历史任务。
