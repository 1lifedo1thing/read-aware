...
本次交接（第73批）：用户解锁，CUA真实系统选择器导入.fb2.zip并打开正文；
宽/高图片旋转自然完成且完整适配，Tab进入灯箱按钮、Escape关闭返回阅读焦点。
两自有书/源/封面及清理意图0，原两书/会话hash保持。无产品改动。
最新隔离发布包已构建，SHA256 603dfe1a…，包含69/70；CUA按新路径启动却
出现旧包PID13551，需核对/解决同bundle ID启动路径后验收，不能记作新包通过。
primary15611/9224保持；旧86059已正常退出。下一步最新packaged、插件复合
过滤、选择/拖动等桌面未覆边界；全格式变体/其他平台/跨设备仍待验。
