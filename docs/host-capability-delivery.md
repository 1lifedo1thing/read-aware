...
真实桌面已新增二十四组组合证据：正文准备/历史、图片脚注、RSS缓存及重排、
版本化标注、记忆分页/纠错/遗忘、设置事务/观察、分句及系统语音回退、
记忆巩固冲突及图谱围栏、原生文件/目录/导出/重复导入、备份UI恢复/重载屏障、
Jumper导航与历史、实时视图订阅/草稿/退役、时长结算及正文状态、正文任务共享/取消/退役、Text Desk实际远端识图、画像/合并身份/上下文归档导出与失效、会话隔离/环境快照/词典缓存跟随、网络重试/延迟执行/流量隔离、回调生命周期/文档保真/合成凭据加密事件、插件数据事务/实际升级回滚/降级/卸载、七种格式导入/正文及加密MOBI拒绝读取、解锁后PDF导航/面板焦点、应用导航/集合多选/宿主命令权限与持久设置、书目/收藏/集合管理与批准删除/恢复后清理保护。具体范围见[验收记录](./evidence/2026-09-13-full-validation/README.md)。
用户已解锁，前台选区和朗读模式截图有效；原生选择器可操作。首次后台选区
消失在新书后台→前台复验中未重现，原观察仍保留；中途恢复了FB2与PDF截图，随后PDF复验时WebView又hidden；用户再次确认解锁后unminimize恢复visible，完整Worker导航及面板DOM焦点已通过。PDF首次正文快照仍有短暂空值，时序差异待核。第23流程中窗口再次hidden，导航/设置和DOM通过，连续绘制帧采集待验。
