...
当前目标：[九类缺失能力补全](./host-capability-completion-goal.md)，基线 `4657c3d0`。C01按书记忆/图谱/归档、C02本书会话、C03受限工作区/命令已接，真实桌面组合待集中验收。标注因果批次已提交 `50fb201c`。C04本批plugins1.5接memory.observe第二参数，查询共用短写入结算/跨提交重读/错误恢复来源；图谱任务排队/进度/取消/终态/淘汰保留调用context，取消反馈与原执行写来源分开，重试使用新调用者。正文共享工作合并来源，各次暂停/恢复有独立请求身份，旧工作迟到不覆盖新来源；library隐式抽取和图谱runtime正文读端接actor并保留原memory policy/取消。Memory Desk0.16绑定自动发布，独立用户动作保留根上下文。公开记忆A→B→A在all/book授权下拒绝循环，独立用户根及图谱观察跨await取消通过；真实Bun Worker、受控来源/消费者/运行时检查通过，日志及类型记录见目标文档，不作Tauri/SQLite/真实模型语义证据。下一步接阅读设置重排、原生选区/焦点、面板/图片、贡献注册/退休及其他观察来源；reading-ai-actions章节准备/openChat还没传actor，也需接线。observeSession的reaction仍未开放，另余设置目录/凭据失效和第一方自动消费者。C04仍未完成，C05–C09未实施且全部保留。既有证据不归零，下列旧active goal为历史任务。
