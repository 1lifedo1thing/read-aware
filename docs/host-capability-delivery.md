...
当前批次：SYS11目录/拖放、SYS12受控文件关联应用打开均已接，剩余为集中运行验收。CFG10普通凭据的崩溃发布断点也已补。TXT11的PDF引用与源节预算已接；TXT12的PDF/CSS/srcset资源消费已接，模型图片输入已接；AI07调用总输出预算/持久请求历史已接；READ16取消补偿和ANN08读取预算已接；下一步处理MEM04/MEM10/11的记忆断点。
