...
当前试跑 goal（2026-09-12）：补 TXT10/TXT13/ANN08 的共同断点——版本化文本范围
写入高亮/引文笔记并保留为标注来源。范围仅含这条宿主、Agent、Worker 与
Annotation Desk 调用链；当前为实现与接线阶段，集中 Tauri/全格式/跨平台验收后置。
普通无锚点笔记保留，旧标注不伪造当前版本；完成定向验证和提交后结束本 goal，不推送。

当前调用链已实现：`BookTextRange` → 共享领域原文校验 → 事件/SQLite v43
`range_json` → 创建回执与标注读模型；Agent `create_annotation`、Worker 的
annotations 2.1 创建接口与 Annotation Desk 0.5 已传递完整来源。文件版本在原生
事务内复核；虚拟来源绑定/代际在宿主派发前复核。旧记录不补造版本。
已通过来源/授权/取消回执、真实解析器 DOM/PDF 夹具和原生 SQLite 定向检查。
实现与接线的验收条件已满足；本完成单元自动提交，不推送，不自动创建下一 goal。
真实 Tauri/Worker、全格式与跨平台仍后置，TXT10/TXT13/ANN08 整行均不因此关闭。

- OPS08 的限定 goal 已结束，见下方验收结果；其他能力未随之完成。
- 后续 MEM01 原生分页已有定向及桌面证据，整行仍部分，见本文件的
  “2026-09-12 · MEM01 原生记忆分页”记录。
- 交付流程调整已提交 `34eab183`，矩阵状态未因此改变。
  本节维护当前具体断点、有效检查及下一步动作，不把下方历史待办全部并入本任务。
